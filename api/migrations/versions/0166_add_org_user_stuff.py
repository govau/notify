...
def upgrade():
    ...
    op.execute(
        "insert into invite_status_type values ('pending'), ('accepted'), ('cancelled')"
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            'user_id', 'organisation_id', name='uix_user_to_organisation'
        ),
    )

    op.create_unique_constraint(
        None, 'organisation_to_service', columns=['service_id', 'organisation_id']
    )
