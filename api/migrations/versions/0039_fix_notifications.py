...
def upgrade():
    ...
    conn = op.get_bind()
    reset_counts = "update notification_statistics set emails_requested = 0, emails_delivered = 0, emails_failed=0," "sms_requested = 0, sms_delivered = 0, sms_failed=0 where day > '2016-06-30'"
    ...
    for x in res:
        ...
        if x.notification_type == 'sms' and x.status in [
            ...
            sql = (
                "update notification_statistics set sms_requested = sms_requested + 1, "
                "sms_failed = sms_failed + 1 where day = date('{}') and service_id = '{}'".format(
                    created, x.service_id
                )
            )
        if x.notification_type == 'email' and x.status in [
            ...
            sql = (
                "update notification_statistics set emails_requested = emails_requested + 1 "
                " where day = date('{}') and service_id = '{}'".format(
                    created, x.service_id
                )
            )
        if x.notification_type == 'sms' and x.status in [
