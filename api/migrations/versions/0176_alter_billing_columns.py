...
def upgrade():
    op.alter_column(
        'dm_datetime', 'calendar_week', existing_type=sa.INTEGER(), nullable=False
    )
    op.alter_column('dm_datetime', 'day', existing_type=sa.INTEGER(), nullable=False)
    op.alter_column(
        'dm_datetime', 'financial_year', existing_type=sa.INTEGER(), nullable=False
    )


def downgrade():
    op.alter_column(
        'dm_datetime', 'financial_year', existing_type=sa.INTEGER(), nullable=True
    )
    op.alter_column('dm_datetime', 'day', existing_type=sa.INTEGER(), nullable=True)
    op.alter_column(
        'dm_datetime', 'calendar_week', existing_type=sa.INTEGER(), nullable=True
    )
