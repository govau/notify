...
def upgrade():
    ...
    op.drop_index(
        'ix_service_letter_contact_service_id', table_name='service_letter_contacts'
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column(
            'service_sms_sender_id', postgresql.UUID(as_uuid=True), nullable=False
        ),
...
def downgrade():
    op.drop_index(
        op.f('ix_service_sms_senders_service_id'), table_name='service_sms_senders'
    )
    op.create_index(
