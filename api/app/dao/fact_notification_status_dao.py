...
from app.utils import convert_aet_to_utc, convert_utc_to_aet, get_sydney_midnight_in_utc, midnight_n_days_ago, get_sydney_month_from_utc_column


def fetch_notification_status_for_day(process_day, service_id=None):
    start_date = convert_aet_to_utc(datetime.combine(process_day, time.min))
    end_date = convert_aet_to_utc(datetime.combine(process_day + timedelta(days=1), time.min))
...
def update_fact_notification_status(data, process_day):
    ...
    FactNotificationStatus.query.filter(
        FactNotificationStatus.aet_date == process_day
    ).delete()
    for row in data:
        stmt = insert(table).values(
            aet_date=process_day,
...
def fetch_notification_status_for_service_by_month(start_date, end_date, service_id):
    return db.session.query(
        func.date_trunc('month', FactNotificationStatus.aet_date).label('month'),
...
def fetch_notification_status_for_service_for_day(aet_day, service_id):
    return db.session.query(
        ...
        Notification.created_at < get_sydney_midnight_in_utc(aet_day + timedelta(days=1)),
...
def fetch_monthly_template_usage_for_service(start_date, end_date, service_id):
    ...
    stats = db.session.query(
        ...
        extract('year', FactNotificationStatus.aet_date),
...
def get_total_sent_notifications_for_day_and_type(day, notification_type):
    result = db.session.query(
        ...
        FactNotificationStatus.key_type != KEY_TYPE_TEST,
        FactNotificationStatus.aet_date == day,
    ).scalar()
