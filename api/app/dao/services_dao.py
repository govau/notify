...
def dao_fetch_todays_stats_for_service(service_id):
    return (
        _stats_for_service_query(service_id)
        .filter(func.date(Notification.created_at) == date.today())
        .all()
    )
...
def dao_fetch_monthly_historical_stats_for_service(service_id, year):
    ...
    rows = (
        ...
        .group_by(
            NotificationHistory.notification_type, NotificationHistory.status, month
        )
        .order_by(month)
    )
    ...
    months = {
        ...
        for created_date in [datetime(year, month, 1) for month in range(4, 13)]
        + [datetime(year + 1, month, 1) for month in range(1, 4)]
    }

    for notification_type, status, created_date, count in rows:
        months[datetime.strftime(created_date, "%Y-%m")][notification_type][
            status
        ] = count
...
@statsd(namespace='dao')
def fetch_stats_by_date_range_for_all_services(
    start_date, end_date, include_from_test_key=True, only_active=True
):
    ...
    subquery = (
        ...
        .filter(table.created_at >= start_date, table.created_at < end_date)
        .group_by(table.notification_type, table.status, table.service_id)
    )
    if not include_from_test_key:
...
@statsd(namespace='dao')
def fetch_aggregate_stats_by_date_range_for_all_services(
    ...
    query = (
        db.session.query(
            table.notification_type, table.status, func.count(table.id).label('count')
        )
        .filter(table.created_at >= start_date, table.created_at < end_date)
        .group_by(table.notification_type, table.status)
        .order_by(table.notification_type)
    )
...
def dao_suspend_service(service_id):
    ...
    # to ensure that db.session still contains the models when it comes to creating history objects
    service = (
        Service.query.options(joinedload('api_keys'))
        .filter(Service.id == service_id)
        .one()
    )
...
def dao_fetch_active_users_for_service(service_id):
    query = User.query.filter(
        User.user_to_service.any(id=service_id), User.state == 'active'
    )
...
def dao_fetch_monthly_historical_stats_by_template():
    ...
    return (
        ...
        .filter(NotificationHistory.created_at < end_date)
        .group_by(NotificationHistory.template_id, month, year)
        .order_by(year, month)
        .all()
    )
...
def dao_fetch_monthly_historical_usage_by_template_for_service(service_id, year):
    ...
    if fy_start < datetime.now() < fy_end:
        today_results = (
            ...
            .order_by(Notification.template_id)
            .all()
        )
        ...
        for today_result in today_results:
            ...
            for stat in stats:
                if (
                    today_result.template_id == stat.template_id
                    and today_result.month == stat.month
