...
def dao_get_organisation_by_service_id(service_id):
    return (
        Organisation.query.join(Organisation.services).filter_by(id=service_id).first()
    )
...
def dao_update_organisation(organisation_id, **kwargs):
    return Organisation.query.filter_by(id=organisation_id).update(kwargs)
...
def dao_get_users_for_organisation(organisation_id):
    return (
        User.query.filter(
            User.organisations.any(id=organisation_id), User.state == 'active'
        )
