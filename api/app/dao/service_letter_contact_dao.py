...
def dao_get_letter_contacts_by_service_id(service_id):
    letter_contacts = (
        ...
        .filter(ServiceLetterContact.service_id == service_id)
        .order_by(
            desc(ServiceLetterContact.is_default), desc(ServiceLetterContact.created_at)
        )
        .all()
    )
...
def add_letter_contact_for_service(service_id, contact_block, is_default):
    ...
    new_letter_contact = ServiceLetterContact(
        service_id=service_id, contact_block=contact_block, is_default=is_default
    )
    ...
    if is_default:
        ...
        if old_default.id == letter_contact_id:
            raise InvalidRequest(
                "You must have at least one letter contact as the default.", 400
            )
