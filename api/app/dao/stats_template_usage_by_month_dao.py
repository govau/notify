...
def insert_or_update_stats_for_template(template_id, month, year, count):
    ...
    if result == 0:
        monthly_stats = StatsTemplateUsageByMonth(
            template_id=template_id, month=month, year=year, count=count
        )
...
def dao_get_template_usage_stats_by_service(service_id, year):
    return (
        ...
        .join(Template, StatsTemplateUsageByMonth.template_id == Template.id)
        .filter(Template.service_id == service_id)
        .filter(
            or_(
                and_(
        ...
        .order_by(desc(StatsTemplateUsageByMonth.month))
        .all()
    )
