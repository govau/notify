    ...
    try:
        ...
        for month in get_months_for_financial_year(year):
            ...
            if letter_billing_for_month:
                results.extend(
                    _transform_billing_for_month_letters(letter_billing_for_month)
                )
...
def get_yearly_billing_usage_summary(service_id):
    try:
        ...
        response = [
            _get_total_billable_units_and_rate_for_notification_type(
                billing_data, notification_type
            )
        ...
        for monthly_total in entry.monthly_totals:
            if entry.notification_type == noti_type:
                if entry.notification_type == EMAIL_TYPE:
                    ...
                    total_sent += (
                        monthly_total['billing_units']
                        * monthly_total['rate_multiplier']
                    )
                    rate = monthly_total['rate']
                elif entry.notification_type == LETTER_TYPE:
                    total_sent += monthly_total['billing_units']
                    letter_total += (
                        monthly_total['billing_units'] * monthly_total['rate']
                    )
