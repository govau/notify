...
def provider_is_primary(current_provider, new_provider, identifier):
    if current_provider.identifier == identifier:
        current_app.logger.warning(
            'Provider {} is already activated'.format(current_provider.display_name)
        )
        return True
...
def _print_provider_switch_logs(current_provider, new_provider):
    current_app.logger.warning(
        'Switching provider from {} to {}'.format(
            current_provider.identifier, new_provider.identifier
        )
    )

    current_app.logger.warning(
        'Provider {} now updated with priority of {}'.format(
    ...
    current_app.logger.warning(
        'Provider {} now updated with priority of {}'.format(
            new_provider.identifier, new_provider.priority
        )
    )
