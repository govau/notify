...
def increment_template_usage_cache(service_id, template_id, created_at):
    key = cache_key_for_service_template_usage_per_day(service_id, convert_utc_to_aest(created_at))
...
def persist_scheduled_notification(notification_id, scheduled_for):
    scheduled_datetime = convert_aest_to_utc(datetime.strptime(scheduled_for, "%Y-%m-%d %H:%M"))
