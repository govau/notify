...
sms_callback_blueprint = Blueprint(
    "sms_callback", __name__, url_prefix="/notifications/sms"
)
...
def process_telstra_response(notification_id):
    ...
    errors = validate_callback_data(
        data=data, fields=['messageId', 'deliveryStatus'], client_name=client_name
    )
    if errors:
...
def process_twilio_response(notification_id):
    ...
    data = request.values
    errors = validate_callback_data(
        data=data, fields=['MessageStatus', 'MessageSid'], client_name=client_name
    )
...
def process_mmg_response():
    ...
    data = json.loads(request.data)
    errors = validate_callback_data(
        data=data, fields=['status', 'CID'], client_name=client_name
    )
    if errors:
        raise InvalidRequest(errors, status_code=400)
    ...
    current_app.logger.debug(
        "Full delivery response from {} for notification: {}\n{}".format(
            client_name, request.form.get('CID'), redacted_data
        )
    )
    if errors:
        raise InvalidRequest(errors, status_code=400)
...
def process_firetext_response():
    ...
    errors = validate_callback_data(
        data=request.form, fields=['status', 'reference'], client_name=client_name
    )
    if errors:
        raise InvalidRequest(errors, status_code=400)
    ...
    current_app.logger.debug(
        "Full delivery response from {} for notification: {}\n{}".format(
            client_name, request.form.get('reference'), redacted_data
        )
    )
    success, errors = process_sms_client_response(
