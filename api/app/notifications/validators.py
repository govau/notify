...
def service_can_send_to_recipient(
    send_to, key_type, service, allow_whitelisted_recipients=True
):
    if not service_allowed_to_send_to(
        send_to, service, key_type, allow_whitelisted_recipients
    ):
...
def validate_and_format_recipient(
    send_to, key_type, service, notification_type, allow_whitelisted_recipients=True
):
    if send_to is None:
        raise BadRequestError(message="Recipient can't be empty")

    service_can_send_to_recipient(
        send_to, key_type, service, allow_whitelisted_recipients
    )
...
def check_sms_content_char_count(content_count):
    ...
    if content_count > char_count_limit:
        message = 'Content for template has a character count greater than the limit of {}'.format(
            char_count_limit
        )
        raise BadRequestError(message=message)
...
def check_service_sms_sender_id(service_id, sms_sender_id, notification_type):
    if sms_sender_id:
        try:
            ...
            message = 'sms_sender_id {} does not exist in database for service id {}'.format(
...
def check_service_letter_contact_id(service_id, letter_contact_id, notification_type):
    if letter_contact_id:
        try:
            return dao_get_letter_contact_by_id(
