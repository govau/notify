...
def process_ses_response(ses_request):
    ...
    try:
        errors = validate_callback_data(
            data=ses_request, fields=['Message'], client_name=client_name
        )
        if errors:
            return errors
        ...
        errors = validate_callback_data(
            data=ses_message, fields=['notificationType'], client_name=client_name
        )
        if errors:
            return errors
        ...
        if notification_type == 'Bounce':
            ...
            if ses_message['bounce']['bounceType'] == 'Permanent':
                notification_type = ses_message['bounce'][
                    'bounceType'
                ]  # permanent or not
            else:
        ...
        try:
            ...
            if not notification:
                warning = (
                    "SES callback failed: notification either not found or already updated "
                    "from sending. Status {} for notification reference {}".format(
                        notification_status, reference
                    )
                )
            ...
            if not aws_response_dict['success']:
                ...
                current_app.logger.info(
                    '{} callback return status of {} for notification: {}'.format(
                        client_name, notification_status, notification.id
                    )
                )
