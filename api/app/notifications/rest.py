...
def send_notification(notification_type):
    ...
    if notification_type not in [SMS_TYPE, EMAIL_TYPE]:
        ...
        msg = (
            msg + ", please use the latest version of the client"
    ...
    if notification_type == SMS_TYPE:
        _service_can_send_internationally(
    ...
    return (
        jsonify(
            data=get_notification_return_data(
                notification_model.id, notification_form, template_object
            )
        ),
        201,
    )
...
def _service_can_send_internationally(service, number):
    ...
    if international_phone_info.international and INTERNATIONAL_SMS_TYPE not in [
        ...
        raise InvalidRequest(
            {'to': ["Cannot send to international mobile numbers"]}, status_code=400
        )
...
def create_template_object_for_notification(template, personalisation):
    ...
    if (
        ...
        and template_object.content_count
