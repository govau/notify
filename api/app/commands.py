...
@click.option(
    '-d', '--valid_from', required=True, type=click_dt(format='%Y-%m-%dT%H:%M:%S')
)
...
def backfill_notification_statuses():
    ...
    """
    LIMIT = 250000
    subq = "SELECT id FROM notification_history WHERE notification_status is NULL LIMIT {}".format(
        LIMIT
    )
    update = "UPDATE notification_history SET notification_status = status WHERE id in ({})".format(
        subq
    )
...
def update_notification_international_flag():
    ...
    update = "update notifications set international = False where id in ({})".format(
        subq
    )
    result = db.session.execute(subq).fetchall()
    ...
    subq_history = (
        "select id from notification_history where international is null limit 250000"
    )
    update_history = "update notification_history set international = False where id in ({})".format(
        subq_history
    )
...
def fix_notification_statuses_not_in_sync():
    ...
    subq = "SELECT id FROM notifications WHERE cast (status as text) != notification_status LIMIT {}".format(
        MAX
    )
    update = "UPDATE notifications SET notification_status = status WHERE id in ({})".format(
        subq
    )
    result = db.session.execute(subq).fetchall()
    ...
    subq_hist = "SELECT id FROM notification_history WHERE cast (status as text) != notification_status LIMIT {}".format(
        MAX
    )
    update = "UPDATE notification_history SET notification_status = status WHERE id in ({})".format(
        subq_hist
...
def populate_monthly_billing(year):
    ...
    def populate(service_id, year, month):
        ...
        print(
            "Finished populating data for {} for service id {}".format(
...
def backfill_performance_platform_totals(start_date, end_date):
    ...
    print(
        'Sending total messages sent for all days between {} and {}'.format(
            start_date, end_date
        )
    )
...
def backfill_processing_time(start_date, end_date):
    ...
    print(
        'Sending notification processing-time data for all days between {} and {}'.format(
            start_date, end_date
        )
    )
...
def populate_annual_billing():
    ...
    for fy in financial_year:
        ...
        """.format(
            fy, datetime.utcnow(), datetime.utcnow(), fy
        )
        ...
        print(
            "Populated annual billing {} for {} services".format(
                fy, services_result1.rowcount
            )
        )
...
def replay_create_pdf_letters(notification_id):
    print(
        "Create task to create_letters_pdf for notification: {}".format(notification_id)
    )
...
def migrate_data_to_ft_billing(start_date, end_date):
    ...
    while process_date < end_date:
        ...
        sql = """
            ...
            """
        num_notifications = db.session.execute(
        ...
        predicted_records = db.session.execute(
            sql, {"start": process_date, "end": process_date + timedelta(days=1)}
        ).fetchall()[0][0]

        start_time = datetime.now()
        print(
            'ft_billing: Migrating date: {}, notifications: {}, expecting {} ft_billing rows'.format(
                process_date.date(), num_notifications, predicted_records
            )
        )
        ...
        db.session.commit()
        print(
            'ft_billing: --- Completed took {}ms. Migrated {} rows.'.format(
                datetime.now() - start_time, result.rowcount
            )
        )
        if predicted_records != result.rowcount:
            print(
                '          : ^^^ Result mismatch by {} rows ^^^'.format(
                    predicted_records - result.rowcount
                )
            )
...
def populate_redis_template_usage(service_id, day):
    ...
    usage = {
        ...
        for row in db.session.query(
            Notification.template_id, func.count().label('count')
        )
        .filter(
