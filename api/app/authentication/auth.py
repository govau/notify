...
def requires_admin_auth():
    ...
    if client == current_app.config.get('ADMIN_CLIENT_USER_NAME'):
        ...
        return handle_admin_key(
            auth_token, current_app.config.get('ADMIN_CLIENT_SECRET')
        )
    else:
...
def requires_auth():
    ...
    for api_key in service.api_keys:
        ...
        raise AuthError(
            "Invalid token: signature, api token not found", 403, service_id=service.id
        )
...
def handle_admin_key(auth_token, secret):
    try:
        ...
        raise AuthError(
            "Invalid token: expired, check that your system clock is accurate", 403
        )
    except TokenDecodeError as e:
        raise AuthError("Invalid token: signature, api token is not valid", 403)
