...
def update_template(service_id, template_id):
    ...
    if not service_has_permission(
        fetched_template.template_type, fetched_template.service.permissions
    ):
    ...
    if "reply_to" in data:
        ...
        updated = dao_update_template_reply_to(
            template_id=template_id, reply_to=data.get("reply_to")
        )
    ...
    if template.is_precompiled_letter:
        ...
        if file_type == 'png':
            try:
                ...
                pdf_page = extract_page_from_pdf(
                    BytesIO(pdf_file), int(page_number) - 1
                )
