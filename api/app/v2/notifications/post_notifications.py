...
def post_precompiled_letter_notification():
    ...
    resp = {'id': notification.id, 'reference': notification.client_reference}
...
def post_notification(notification_type):
    ...
    check_service_can_schedule_notification(
        authenticated_service.permissions, scheduled_for
    )
...
def process_sms_or_email_notification(
    *, form, notification_type, api_key, template, service, reply_to_text=None
):
    form_send_to = (
        form['email_address']
        if notification_type == EMAIL_TYPE
    ...
    if scheduled_for:
        ...
        if not simulated:
            queue_name = (
                QueueNames.PRIORITY if template.process_type == PRIORITY else None
            )
            send_notification_to_queue(
                notification=notification,
            ...
            current_app.logger.debug(
                "POST simulated notification for id: {}".format(notification.id)
            )
...
def process_letter_notification(
    *, letter_data, api_key, template, reply_to_text, precompiled=False
):
    ...
    if (
        not api_key.service.research_mode
        and api_key.service.restricted
        and api_key.key_type != KEY_TYPE_TEST
    ):
        raise BadRequestError(
            message='Cannot send letters when service is in trial mode', status_code=403
        )
    ...
    if should_send:
        ...
        create_fake_letter_response_file.apply_async(
            (notification.reference,), queue=QueueNames.RESEARCH_MODE
        )
    else:
        update_notification_status_by_reference(
            notification.reference, NOTIFICATION_DELIVERED
        )
...
def process_precompiled_letter_notifications(
    *, letter_data, api_key, template, reply_to_text
):
    try:
        ...
        raise BadRequestError(
            message='Letter content is not a valid PDF', status_code=400
        )
...
def get_precompiled_letter_template(service_id):
    template = Template.query.filter_by(
        service_id=service_id, template_type=LETTER_TYPE, hidden=True
    ).first()
