...
def update_letter_notifications_statuses(self, filename):
    ...
    try:
        ...
        try:
            ...
            billing_date = get_billing_date_in_aest_from_filename(filename)
...
def get_billing_date_in_aest_from_filename(filename):
    datetime_string = filename.split('-')[1]
    datetime_obj = datetime.strptime(datetime_string, '%Y%m%d%H%M%S')
    return convert_utc_to_aest(datetime_obj).date()
