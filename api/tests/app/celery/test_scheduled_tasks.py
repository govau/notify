...
def _create_slow_delivery_notification(provider='mmg'):
    ...
    service = Service.query.get(
        current_app.config['FUNCTIONAL_TEST_PROVIDER_SERVICE_ID']
    )
    if not service:
...
@pytest.mark.skip(
    reason="This doesn't actually test the celery task wraps the function"
)
def test_should_have_decorated_tasks_functions():
    ...
    assert (
        send_daily_performance_platform_stats.__wrapped__.__name__
        == 'send_daily_performance_platform_stats'
    )
    assert (
        switch_current_sms_provider_on_slow_delivery.__wrapped__.__name__
        == 'switch_current_sms_provider_on_slow_delivery'
    )
    assert (
        delete_inbound_sms_older_than_seven_days.__wrapped__.__name__
        == 'delete_inbound_sms_older_than_seven_days'
    )
    assert (
        remove_transformed_dvla_files.__wrapped__.__name__
        == 'remove_transformed_dvla_files'
    )
    assert (
        delete_dvla_response_files_older_than_seven_days.__wrapped__.__name__
        == 'delete_dvla_response_files_older_than_seven_days'
    )
    assert populate_monthly_billing.__wrapped__.__name__ == 'populate_monthly_billing'
...
def test_should_call_delete_sms_notifications_more_than_week_in_task(
    notify_api, mocker
):
    mocked = mocker.patch(
...
def test_should_call_delete_email_notifications_more_than_week_in_task(
    notify_api, mocker
):
...
def test_should_call_delete_letter_notifications_more_than_week_in_task(
    notify_api, mocker
):
    mocked = mocker.patch(
        'app.celery.scheduled_tasks.delete_notifications_created_more_than_a_week_ago_by_type'
...
def test_should_not_update_status_of_letter_notifications(
    ...
    not1 = create_notification(
        template=sample_letter_template, status='sending', created_at=created_at
    )
    not2 = create_notification(
        template=sample_letter_template, status='created', created_at=created_at
    )
...
def test_timeout_notifications_sends_status_update_to_service(
    ...
    mocked.assert_called_once_with(
        [str(notification.id), encrypted_data], queue=QueueNames.CALLBACKS
    )


def test_should_update_scheduled_jobs_and_put_on_queue(
...
def test_will_remove_csv_files_for_jobs_older_than_seven_days(
    ...
    job1_to_delete = create_sample_job(
        notify_db, notify_db_session, created_at=eight_days_ago
    )
    job2_to_delete = create_sample_job(
        notify_db, notify_db_session, created_at=just_under_nine_days
    )
...
def test_send_daily_performance_stats_calls_does_not_send_if_inactive(client, mocker):
    send_mock = mocker.patch(
        'app.celery.scheduled_tasks.total_sent_notifications.send_total_notifications_sent_for_day_stats'
    )  # noqa

    with patch.object(
        PerformancePlatformClient, 'active', new_callable=PropertyMock
    ) as mock_active:
...
def test_send_total_sent_notifications_to_performance_platform_calls_with_correct_totals(
    notify_db, notify_db_session, sample_template, mocker
):
...
def test_switch_providers_on_slow_delivery_runs_if_config_set(
    notify_api, mocker, prepare_current_provider
):
    get_notifications_mock = mocker.patch(
        'app.celery.scheduled_tasks.is_delivery_slow_for_provider', return_value=[]
    )
...
def test_switch_providers_triggers_on_slow_notification_delivery(
    ...
    mocker.patch(
        'app.provider_details.switch_providers.get_user_by_id', return_value=sample_user
    )
...
def test_should_send_all_scheduled_notifications_to_deliver_queue(
    ...
    message_to_deliver = create_notification(
    ...
    mocked.apply_async.assert_called_once_with(
        [str(message_to_deliver.id)], queue='send-sms-tasks'
    )
    scheduled_notifications = dao_get_scheduled_notifications()
...
def test_should_call_delete_inbound_sms_older_than_seven_days(notify_api, mocker):
    mocker.patch(
        'app.celery.scheduled_tasks.delete_inbound_sms_created_more_than_a_week_ago'
    )
    delete_inbound_sms_older_than_seven_days()
    assert (
        scheduled_tasks.delete_inbound_sms_created_more_than_a_week_ago.call_count == 1
    )
...
def test_delete_dvla_response_files_older_than_seven_days_removes_old_files(
    ...
    mocker.patch(
        'app.celery.scheduled_tasks.s3.get_s3_bucket_objects',
...
def test_alert_if_letter_notifications_still_sending(sample_letter_template, mocker):
    ...
    create_notification(
        template=sample_letter_template, status='sending', sent_at=yesterday
    )

    mock_celery = mocker.patch(
        "app.celery.scheduled_tasks.deskpro_client.create_ticket"
    )
...
@freeze_time("2018-01-17 17:00:00")
def test_alert_if_letter_notifications_still_sending_only_alerts_sending(
    ...
    create_notification(
        template=sample_letter_template, status='delivered', sent_at=yesterday
    )
    create_notification(
        template=sample_letter_template, status='failed', sent_at=yesterday
    )
...
@freeze_time("2018-01-17 17:00:00")
def test_alert_if_letter_notifications_still_sending_only_alerts_previous_day(
    sample_letter_template, mocker
):
    day_before_yesterday = datetime(2018, 1, 15, 13, 30)
    create_notification(
        template=sample_letter_template, status='sending', sent_at=day_before_yesterday
    )
...
def test_monday_alert_if_letter_notifications_still_sending_reports_friday_letters(
    sample_letter_template, mocker
):
    ...
    create_notification(
        template=sample_letter_template, status='sending', sent_at=friday
    )
    create_notification(
        template=sample_letter_template, status='sending', sent_at=yesterday
    )
...
def test_populate_monthly_billing_populates_correctly(sample_template):
    ...
    create_notification(
        template=sample_template, status='delivered', created_at=yesterday
    )
    create_notification(
        template=sample_template,
        status='delivered',
    ...
    create_notification(
        template=sample_template,
        status='delivered',
        created_at=yesterday - timedelta(days=30),
    )
...
def test_run_letter_jobs(client, mocker, sample_letter_template):
    jobs = [
        create_job(
            template=sample_letter_template, job_status=JOB_STATUS_READY_TO_SEND
        ),
        create_job(
            template=sample_letter_template, job_status=JOB_STATUS_READY_TO_SEND
        ),
    ...
    mock_celery.assert_called_once_with(
        name=TaskNames.DVLA_JOBS, args=(job_ids,), queue=QueueNames.PROCESS_FTP
    )
...
def test_daily_stats_template_usage_by_month(notify_db, notify_db_session):
    notification_history = functools.partial(
        create_notification_history, notify_db, notify_db_session, status='delivered'
    )
    ...
    result = (
        ...
        .order_by(StatsTemplateUsageByMonth.year, StatsTemplateUsageByMonth.month)
        .all()
    )
...
def test_daily_stats_template_usage_by_month_multiple_runs(
    ...
    notification_history(
        created_at=datetime(2017, 10, 1), sample_template=template_three
    )
    notification_history(
        created_at=datetime(2017, 9, 1), sample_template=template_three
    )
    notification_history(created_at=datetime(2016, 4, 1), sample_template=template_two)
    notification_history(created_at=datetime(2016, 4, 1), sample_template=template_two)
    notification_history(created_at=datetime.now(), sample_template=template_two)
...
def test_dao_fetch_monthly_historical_stats_by_template_null_template_id_not_counted(
    notify_db, notify_db_session
):
    notification_history = functools.partial(
        create_notification_history, notify_db, notify_db_session, status='delivered'
    ...
    template_one = create_sample_template(
        notify_db, notify_db_session, template_name='1'
    )
    history = notification_history(
        created_at=datetime(2017, 2, 1), sample_template=template_one
    )

    NotificationHistory.query.filter(NotificationHistory.id == history.id).update(
        {'template_id': None}
    )

    daily_stats_template_usage_by_month()

    result = db.session.query(StatsTemplateUsageByMonth).all()
...
def mock_s3_get_list_diff(bucket_name, subfolder='', suffix='', last_modified=None):
    if subfolder == '2018-01-11/zips_sent':
        return [
            'NOTIFY.20180111175007.ZIP.TXT',
            'NOTIFY.20180111175008.ZIP.TXT',
            'NOTIFY.20180111175009.ZIP.TXT',
...
def test_letter_not_raise_alert_if_ack_files_match_zip_list(mocker, notify_db):
    mock_file_list = mocker.patch(
        "app.aws.s3.get_list_of_files_by_suffix", side_effect=mock_s3_get_list_match
    )
    mock_get_file = mocker.patch(
        "app.aws.s3.get_s3_file",
...
def test_letter_raise_alert_if_ack_files_not_match_zip_list(mocker, notify_db):
    mock_file_list = mocker.patch(
        "app.aws.s3.get_list_of_files_by_suffix", side_effect=mock_s3_get_list_diff
    )
    mock_get_file = mocker.patch(
        "app.aws.s3.get_s3_file",
        return_value='NOTIFY.20180111175007.ZIP|20180111175733\n'
    ...
    deskpro_message = (
        "Letter ack file does not contains all zip files sent. "
        "Missing ack for zip files: {}, "
        "pdf bucket: {}, subfolder: {}, "
        "ack bucket: {}".format(
            str(['NOTIFY.20180111175009.ZIP', 'NOTIFY.20180111175010.ZIP']),
    ...
    mock_deskpro.assert_called_once_with(
        subject="Letter acknowledge error", message=deskpro_message, ticket_type='alert'
    )
...
def test_letter_not_raise_alert_if_no_files_do_not_cause_error(mocker, notify_db):
    mock_file_list = mocker.patch(
        "app.aws.s3.get_list_of_files_by_suffix", side_effect=None
    )
    mock_get_file = mocker.patch(
        "app.aws.s3.get_s3_file",
        return_value='NOTIFY.20180111175007.ZIP|20180111175733\n'
