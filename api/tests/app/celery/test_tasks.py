...
def email_job_with_placeholders(
    ...
    return create_sample_job(
        notify_db, notify_db_session, template=sample_email_template_with_placeholders
    )
...
def test_should_process_sms_job(sample_job, mocker):
    mocker.patch(
        'app.celery.tasks.s3.get_job_from_s3', return_value=load_example_csv('sms')
    )
    ...
    s3.get_job_from_s3.assert_called_once_with(
        str(sample_job.service.id), str(sample_job.id)
    )
...
def test_should_not_process_sms_job_if_would_exceed_send_limits(
    ...
    job = create_sample_job(
        notify_db, notify_db_session, service=service, notification_count=10
    )
...
def test_should_not_process_email_job_if_would_exceed_send_limits_inc_today(
    ...
    service = create_sample_service(notify_db, notify_db_session, limit=1)
    template = create_sample_email_template(
...
@freeze_time("2016-01-01 11:09:00.061258")
def test_should_not_process_email_job_if_would_exceed_send_limits(
    notify_db, notify_db_session, mocker
):
...
def test_should_not_create_save_task_for_empty_file(sample_job, mocker):
    mocker.patch(
        'app.celery.tasks.s3.get_job_from_s3', return_value=load_example_csv('empty')
    )
    mocker.patch('app.celery.tasks.save_sms.apply_async')
...
def test_should_process_letter_job(sample_letter_job, mocker):
    ...
    s3_mock.assert_called_once_with(
        str(sample_letter_job.service.id), str(sample_letter_job.id)
    )
...
def test_should_put_save_sms_task_in_research_mode_queue_if_research_mode_service(
    ...
    save_sms(template.service_id, notification_id, encryption.encrypt(notification))
    persisted_notification = Notification.query.one()
...
def test_should_save_sms_if_restricted_service_and_valid_number(
    ...
    service = create_sample_service(
        notify_db, notify_db_session, user=user, restricted=True
    )
    template = create_sample_template(notify_db, notify_db_session, service=service)
    notification = _notification_json(
        template, "+61412345678"
    ...
    save_sms(service.id, notification_id, encrypt_notification)
...
def test_should_save_sms_if_restricted_service_and_non_team_number_with_test_key(
    notify_db, notify_db_session, mocker
):
...
def test_should_save_email_if_restricted_service_and_non_team_email_address_with_test_key(
    notify_db, notify_db_session, mocker
):
    user = create_user()
    service = create_sample_service(
    ...
    mocked_deliver_email = mocker.patch(
...
def test_save_sms_should_save_default_smm_sender_notification_reply_to_text_on(
    notify_db_session, mocker
):
...
def test_should_not_save_sms_if_restricted_service_and_invalid_number(
    ...
    notification_id = uuid.uuid4()
    save_sms(service.id, notification_id, encryption.encrypt(notification))
...
def test_should_not_save_email_if_restricted_service_and_invalid_email_address(
    ...
    notification_id = uuid.uuid4()
    save_email(service.id, notification_id, encryption.encrypt(notification))
...
def test_should_put_save_email_task_in_research_mode_queue_if_research_mode_service(
    ...
    save_email(template.service_id, notification_id, encryption.encrypt(notification))
...
def test_should_not_save_email_if_team_key_and_recipient_not_in_team(
    sample_email_template_with_placeholders, sample_team_api_key, mocker
):
    notification = _notification_json(
    ...
    team_members = [
        user.email_address
        for user in sample_email_template_with_placeholders.service.users
    ]
...
def test_save_email_should_use_template_version_from_job_not_latest(
    ...
    now = datetime.utcnow()
    save_email(
        sample_email_template.service_id, uuid.uuid4(), encryption.encrypt(notification)
    )
...
def test_save_email_does_not_send_duplicate_and_does_not_put_in_retry_queue(
    ...
    save_email(
        sample_notification.service_id, notification_id, encryption.encrypt(json)
    )
...
def test_save_sms_does_not_send_duplicate_and_does_not_put_in_retry_queue(
    ...
    save_sms(sample_notification.service_id, notification_id, encryption.encrypt(json))
    assert Notification.query.count() == 1
...
def test_save_letter_saves_letter_to_database(mocker, notify_db_session):
    ...
    contact_block = create_letter_contact(
        service=service, contact_block="Address contact", is_default=True
    )
    template = create_template(
        service=service, template_type=LETTER_TYPE, reply_to=contact_block.id
    )
    ...
    save_letter(job.service_id, notification_id, encryption.encrypt(notification_json))
...
def test_save_letter_saves_letter_to_database_right_reply_to(mocker, notify_db_session):
    ...
    template = create_template(
        service=service, template_type=LETTER_TYPE, reply_to=None
    )
    job = create_job(template=template)
...
def test_save_letter_uses_template_reply_to_text(mocker, notify_db_session):
    ...
    template = create_template(
        service=service, template_type=LETTER_TYPE, reply_to=template_contact.id
    )
    ...
    save_letter(job.service_id, uuid.uuid4(), encryption.encrypt(notification_json))
...
def test_save_letter_sets_delivered_letters_as_pdf_permission_in_research_mode_in_staging_live(
    ...
    mocker.patch(
        'app.celery.tasks.create_random_identifier', return_value=sample_reference
    )
...
def test_save_letter_calls_create_fake_response_for_letters_in_research_mode_on_development_preview(
    ...
    mock_create_fake_letter_response_file.assert_called_once_with(
        (sample_reference,), queue=QueueNames.RESEARCH_MODE
    )
...
def test_save_letter_calls_create_letters_pdf_task_not_in_research(
    ...
    mock_create_letters_pdf = mocker.patch(
    ...
    mock_create_letters_pdf.assert_called_once_with(
        [str(notification_id)], queue=QueueNames.CREATE_LETTERS_PDF
    )
