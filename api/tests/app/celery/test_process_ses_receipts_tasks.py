...
def test_process_ses_results(sample_email_template):
    ...
    assert process_ses_results_task(response=ses_notification_callback(reference='ref1'))
...
def test_ses_callback_should_update_multiple_notification_status_sent(
    ...
    assert process_ses_results_task(ses_notification_callback(reference='ref1'))
    assert process_ses_results_task(ses_notification_callback(reference='ref2'))
    assert process_ses_results_task(ses_notification_callback(reference='ref3'))
...
def test_ses_callback_should_set_status_to_temporary_failure(client,
    ...
    assert get_notification_by_id(notification.id).status == 'sending'
    assert process_ses_results_task(ses_soft_bounce_callback(reference='ref'))
...
def test_ses_callback_should_send_on_complaint_to_user_callback_api(sample_email_template, mocker):
    ...
    assert process_ses_results_task(response)
