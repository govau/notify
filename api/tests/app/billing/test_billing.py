...
def test_get_yearly_billing_summary_returns_correct_breakdown(client, sample_template):
    create_rate(
        start_date=IN_MAY_2016 - timedelta(days=1),
        value=0.12,
    ...
    letter_template = create_template(
        service=sample_template.service, template_type=LETTER_TYPE
    )
    create_notification(
    ...
    create_or_update_monthly_billing(
        service_id=sample_template.service_id, billing_month=IN_MAY_2016
    )
    create_or_update_monthly_billing(
        service_id=sample_template.service_id, billing_month=IN_JUN_2016
    )
...
def test_get_yearly_billing_usage_breakdown_returns_400_if_missing_year(
...
def test_get_yearly_usage_by_month_returns_empty_list_if_no_usage(
    ...
    response = client.get(
        '/service/{}/billing/monthly-usage?year=2016'.format(
            sample_template.service.id
        ),
        headers=[create_authorization_header()],
    )
...
def test_create_free_sms_fragment_limit_current_year_updates_future_years(
    ...
    current_billing = dao_get_free_sms_fragment_limit_for_year(
        sample_service.id, current_year
    )
...
def test_create_free_sms_fragment_limit_updates_existing_year(
    admin_request, sample_service
):
    current_year = get_current_financial_year_start_year()
