...
from app.models import Notification, User, VerifyCode, EMAIL_TYPE, SMS_TYPE
from app import db
...
def test_send_user_sms_code(
    client, sample_user, sms_code_template, mocker, research_mode
):
    """
...
def test_send_new_user_email_verification(
    client, sample_user, mocker, email_verification_template
):
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ({'to': None}, 'http://localhost:6012/email-auth/.'),
        (
...
def test_send_user_email_code(
    admin_request, mocker, sample_user, email_2fa_code_template, data, expected_auth_url
):
