...
def test_create_provider_rates(notify_db, notify_db_session, twilio_provider):
    now = datetime.now()
    rate = Decimal("1.00000")

    provider = ProviderDetails.query.filter_by(identifier=twilio_provider.identifier).one()

    create_provider_rates(twilio_provider.identifier, now, rate)
