...
def _assert_monthly_billing(
    monthly_billing, service_id, notification_type, month_start, month_end
):
...
def test_add_monthly_billing_for_single_month_populates_correctly(
    ...
    create_notification(
        template=letter_template, created_at=JAN_2017_MONTH_START, status='delivered'
    )

    create_or_update_monthly_billing(
        service_id=sample_template.service_id, billing_month=JAN_2017_MONTH_START
    )
...
def test_add_monthly_billing_for_multiple_months_populate_correctly(sample_template):
    create_rate(
        start_date=FEB_2016_MONTH_START - timedelta(days=1),
        value=0.12,
        notification_type=SMS_TYPE,
    )
    create_notification(
    ...
    create_or_update_monthly_billing(
        service_id=sample_template.service_id, billing_month=FEB_2016_MONTH_START
    )
    create_or_update_monthly_billing(
        service_id=sample_template.service_id, billing_month=MAR_2016_MONTH_START
    )
...
def test_add_monthly_billing_with_multiple_rates_populate_correctly(
    ...
    create_rate(
        start_date=JAN_2017_MONTH_START + timedelta(days=5),
        value=0.123,
        notification_type=SMS_TYPE,
    )
    create_notification(
    ...
    create_notification(
        template=sample_template,
        created_at=JAN_2017_MONTH_START + timedelta(days=6),
        billable_units=2,
        status='delivered',
    )
    ...
    _assert_monthly_billing_totals(
        monthly_billing[1].monthly_totals[1],
        {
            "billing_units": 2,
            "rate_multiplier": 1,
            "international": False,
...
def test_update_monthly_billing_overwrites_old_totals(sample_template):
    ...
    with freeze_time(APR_2016_MONTH_START + timedelta(days=3)):
        create_notification(
            template=sample_template, billable_units=2, status='delivered'
        )
        create_or_update_monthly_billing(sample_template.service_id, APR_2016_MONTH_END)
...
def test_get_service_ids_that_need_billing_populated_return_correctly(
    notify_db_session
):
    ...
    create_notification(
        template=template_1,
        created_at=datetime(2017, 6, 30, 13, 30),
        status='delivered',
    )
    create_notification(
        template=template_1, created_at=datetime(2017, 7, 1, 14, 30), status='delivered'
    )
...
def test_get_yearly_billing_data_for_year_returns_within_year_only(sample_template):
    ...
    billing_data = get_billing_data_for_financial_year(
        sample_template.service.id, 2016, [SMS_TYPE]
    )
...
def test_get_yearly_billing_data_for_year_returns_multiple_notification_types(
    sample_template
):
    monthly_billing_entry = partial(
        create_monthly_billing_entry,
    ...
    monthly_billing_entry(notification_type=SMS_TYPE, monthly_totals=[])
    monthly_billing_entry(
        notification_type=EMAIL_TYPE,
        monthly_totals=[
            {
...
def test_get_yearly_billing_data_for_year_includes_current_day_totals(sample_template):
    create_rate(
        start_date=FEB_2016_MONTH_START, value=0.0158, notification_type=SMS_TYPE
    )
    ...
    billing_data = get_billing_data_for_financial_year(
        service_id=sample_template.service.id, year=2016, notification_types=[SMS_TYPE]
    )
