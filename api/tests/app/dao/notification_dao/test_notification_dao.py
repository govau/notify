...
def test_should_have_decorated_notifications_dao_functions():
    assert (
        dao_get_last_template_usage.__wrapped__.__name__
        == 'dao_get_last_template_usage'
    )  # noqa
    assert (
        dao_get_template_usage.__wrapped__.__name__ == 'dao_get_template_usage'
    )  # noqa
    assert (
        dao_create_notification.__wrapped__.__name__ == 'dao_create_notification'
    )  # noqa
    assert (
        update_notification_status_by_id.__wrapped__.__name__
        == 'update_notification_status_by_id'
    )  # noqa
    assert (
        dao_update_notification.__wrapped__.__name__ == 'dao_update_notification'
    )  # noqa
    assert (
        update_notification_status_by_reference.__wrapped__.__name__
        == 'update_notification_status_by_reference'
    )  # noqa
    assert (
        get_notification_for_job.__wrapped__.__name__ == 'get_notification_for_job'
    )  # noqa
    assert (
        get_notifications_for_job.__wrapped__.__name__ == 'get_notifications_for_job'
    )  # noqa
    assert (
        get_notification_with_personalisation.__wrapped__.__name__
        == 'get_notification_with_personalisation'
    )  # noqa
    assert (
        get_notifications_for_service.__wrapped__.__name__
        == 'get_notifications_for_service'
    )  # noqa
    assert (
        get_notification_by_id.__wrapped__.__name__ == 'get_notification_by_id'
    )  # noqa
    assert (
        delete_notifications_created_more_than_a_week_ago_by_type.__wrapped__.__name__
        == 'delete_notifications_created_more_than_a_week_ago_by_type'
    )  # noqa
    assert (
        dao_delete_notifications_and_history_by_id.__wrapped__.__name__
        == 'dao_delete_notifications_and_history_by_id'
    )  # noqa


def test_should_be_able_to_get_template_usage_history(
    ...
    with freeze_time('2000-01-01 12:00:00'):
        ...
        notification = sample_notification(
            notify_db, notify_db_session, service=sample_service, template=sms
        )
        ...
        assert results.created_at == datetime(
...
def test_should_be_able_to_get_all_template_usage_history_order_by_notification_created_at(
    notify_db, notify_db_session, sample_service, notification_type
):
    template = create_sample_template(
        notify_db, notify_db_session, template_type=notification_type
    )
    ...
    most_recent = sample_notification(
        notify_db, notify_db_session, service=sample_service, template=template
    )
...
def test_template_usage_should_ignore_test_keys(
    notify_db, notify_db_session, sample_team_api_key, sample_test_api_key
):
    sms = create_sample_template(notify_db, notify_db_session)
...
def test_should_by_able_to_get_template_count(
    notify_db, notify_db_session, sample_service
):
    sms = create_sample_template(notify_db, notify_db_session)
    ...
    sample_notification(
        notify_db, notify_db_session, service=sample_service, template=email
    )
    sample_notification(
        notify_db, notify_db_session, service=sample_service, template=email
    )
...
def test_template_history_should_ignore_test_keys(
    ...
    sample_notification(notify_db, notify_db_session, template=sms)
...
def test_should_by_able_to_get_template_count_limited_for_service(
    ...
    service_1 = sample_service(
        notify_db, notify_db_session, service_name="test1", email_from="test1"
    )
    service_2 = sample_service(
        notify_db, notify_db_session, service_name="test2", email_from="test2"
    )
...
def test_should_by_able_to_get_zero_count_from_notifications_history_if_no_rows(
    sample_service
):
    results = dao_get_template_usage(sample_service.id)
...
def test_should_by_able_to_update_status_by_reference(
    sample_email_template, smtp_provider
):
...
def test_should_by_able_to_update_status_by_id(
    sample_template, sample_job, mmg_provider
):
    with freeze_time('2000-01-01 12:00:00'):
...
def test_should_not_update_status_by_id_if_not_sending_and_does_not_update_job(
    ...
    notification = sample_notification(
        notify_db, notify_db_session, status='delivered', job=job
    )
    assert Notification.query.get(notification.id).status == 'delivered'
...
def test_should_not_update_status_by_reference_if_not_sending_and_does_not_update_job(
    ...
    job = sample_job(notify_db, notify_db_session)
    notification = sample_notification(
        notify_db, notify_db_session, status='delivered', reference='reference', job=job
    )
    assert Notification.query.get(notification.id).status == 'delivered'
...
def test_should_not_update_status_by_reference_if_not_sending(
    ...
    notification = sample_notification(
        notify_db, notify_db_session, status='created', reference='reference'
    )
    assert Notification.query.get(notification.id).status == 'created'
...
def test_should_by_able_to_update_status_by_id_from_pending_to_delivered(
    ...
    assert update_notification_status_by_id(
        notification_id=notification.id, status='pending'
    )
...
def test_should_not_update_status_one_notification_status_is_delivered(
    notify_db, notify_db_session, sample_email_template, smtp_provider
):
    notification = sample_notification(
        notify_db=notify_db,
...
def test_save_notification_with_test_api_key_does_not_create_history(
    sample_email_template, sample_api_key
):
    assert Notification.query.count() == 0
...
def test_update_notification_with_test_api_key_does_not_update_or_create_history(
    sample_email_template, sample_api_key
):
    assert Notification.query.count() == 0
    data = _notification_json(sample_email_template)
...
def test_save_notification_and_increment_correct_job(
    notify_db, notify_db_session, sample_template, mmg_provider
):
...
def test_get_all_notifications_for_job_by_status(
    notify_db, notify_db_session, sample_job
):
    ...
    for status in NOTIFICATION_STATUS_TYPES:
        if status == 'failed':
            assert len(notifications(filter_dict={'status': status}).items) == len(
                NOTIFICATION_STATUS_TYPES_FAILED
            )
        else:
...
@pytest.mark.parametrize(
    'month, delete_run_time', [(4, '2016-04-10 23:40'), (1, '2016-01-11 00:40')]
)
@pytest.mark.parametrize(
...
def test_should_delete_notifications_by_type_after_seven_days(
    ...
    for i in range(1, 11):
        ...
        with freeze_time(past_date):
            ...
            create_notification(
                template=sms_template, created_at=datetime.utcnow(), status="delivered"
            )
            create_notification(
    ...
    remaining_email_notifications = Notification.query.filter_by(
...
def test_should_not_delete_notification_history(
    ...
    with freeze_time('2016-01-01 12:00'):
        email_template = sample_email_template(
...
def test_should_limit_notifications_return_by_day_limit_plus_one(sample_template):
    ...
    for i in range(1, 11):
        ...
        with freeze_time(past_date):
            create_notification(
                sample_template, created_at=datetime.utcnow(), status="failed"
            )
...
def test_should_delete_notification_and_ignore_history_for_test_api(
    notify_db, notify_db_session, sample_email_template, sample_api_key
):
    data = _notification_json(sample_email_template)
    data['key_type'] = KEY_TYPE_TEST
    data['api_key_id'] = sample_api_key.id
...
def test_should_delete_only_notification_and_notification_history_with_id(
    notify_db, notify_db_session, sample_template
):
    id_1 = uuid.uuid4()
    id_2 = uuid.uuid4()
...
def test_get_notifications_with_a_live_api_key_type(
    ...
    all_notifications = get_notifications_for_service(
        sample_service.id, limit_days=1, key_type=KEY_TYPE_NORMAL
    ).items
    assert len(all_notifications) == 1

    # only those created with normal API key, with jobs
    all_notifications = get_notifications_for_service(
        sample_service.id, limit_days=1, include_jobs=True, key_type=KEY_TYPE_NORMAL
    ).items
    assert len(all_notifications) == 2
...
def test_get_notifications_with_a_test_api_key_type(
    ...
    all_notifications = get_notifications_for_service(
        sample_service.id, limit_days=1, key_type=KEY_TYPE_TEST
    ).items
    assert len(all_notifications) == 1

    # only those created with test API key, no jobs, even when requested
    all_notifications = get_notifications_for_service(
        sample_service.id, limit_days=1, include_jobs=True, key_type=KEY_TYPE_TEST
    ).items
    assert len(all_notifications) == 1
...
def test_get_notifications_with_a_team_api_key_type(
    ...
    all_notifications = get_notifications_for_service(
        sample_service.id, limit_days=1, key_type=KEY_TYPE_TEAM
    ).items
    assert len(all_notifications) == 1

    # only those created with team API key, no jobs, even when requested
    all_notifications = get_notifications_for_service(
        sample_service.id, limit_days=1, include_jobs=True, key_type=KEY_TYPE_TEAM
    ).items
    assert len(all_notifications) == 1
...
def test_should_exclude_test_key_notifications_by_default(
    ...
    all_notifications = get_notifications_for_service(
        sample_service.id, limit_days=1
    ).items
    assert len(all_notifications) == 2
...
def test_get_total_sent_notifications_in_date_range_returns_only_in_date_range(
    notify_db, notify_db_session, sample_template, notification_type
):
    ...
    total_count = get_total_sent_notifications_in_date_range(
...
@freeze_time("2016-01-10 12:00:00.000000")
def test_slow_provider_delivery_returns_for_sent_notifications(sample_template):
    now = datetime.utcnow()
...
@freeze_time("2016-01-10 12:00:00.000000")
def test_slow_provider_delivery_observes_threshold(sample_template):
    now = datetime.utcnow()
    five_minutes_from_now = now + timedelta(minutes=5)
...
def test_dao_get_notifications_by_to_field_escapes(
    sample_email_template, search_term, expected_result_count
):
    ...
    assert (
        len(
            dao_get_notifications_by_to_field(
                sample_email_template.service_id, search_term, notification_type='email'
            )
        )
...
def test_dao_get_notifications_by_to_field_matches_partial_phone_numbers(
    sample_template, search_term
):

    notification_1 = create_notification(
        template=sample_template, to_field='+447700900100', normalised_to='447700900100'
    )
    notification_2 = create_notification(
        template=sample_template, to_field='+447700900200', normalised_to='447700900200'
    )
    results = dao_get_notifications_by_to_field(
...
def test_dao_get_notifications_by_to_field_only_searches_for_notification_type(
    ...
    sms = create_notification(
        template=sms_template, to_field='0771111111', normalised_to='0771111111'
    )
    ...
    results = dao_get_notifications_by_to_field(
        service.id, "077", notification_type='sms'
    )
    assert len(results) == 1
    assert results[0].id == sms.id
    results = dao_get_notifications_by_to_field(
        service.id, "077", notification_type='email'
    )
    assert len(results) == 1
...
def test_set_scheduled_notification_to_processed(
    notify_db, notify_db_session, sample_template
):
    notification_1 = sample_notification(
        notify_db=notify_db,
...
def test_dao_update_notifications_by_reference_updated_notificaitons_and_history(
    sample_template
):
...
def test_dao_get_notification_by_reference_with_one_match_returns_notification(
    sample_letter_template, notify_db
):
...
def test_dao_get_count_of_letters_to_process_for_today(sample_letter_template):
    # expected
    create_notification(
        template=sample_letter_template, created_at='2017-12-17 17:30:00'
    )
    create_notification(
        template=sample_letter_template, created_at='2017-12-18 17:29:59'
    )

    # not expected
    create_notification(
        template=sample_letter_template, created_at='2017-12-17 17:29:59'
    )
    create_notification(
        template=sample_letter_template, created_at='2017-12-18 17:30:00'
    )
...
def test_dao_get_count_of_letters_to_process_for_date_in_past(sample_letter_template):
    # expected
    create_notification(
        template=sample_letter_template, created_at='2017-12-15 17:29:59'
    )

    # not expected
    create_notification(
        template=sample_letter_template, created_at='2017-12-15 17:30:00'
    )
    create_notification(
        template=sample_letter_template, created_at='2017-12-18 17:29:00'
    )
...
@freeze_time("2017-12-18 17:50")
def test_dao_get_count_of_letters_to_process_for_date_in_future_does_not_raise_error(
    sample_letter_template
):
    # not expected
    create_notification(
    ...
    create_notification(
        template=sample_letter_template, created_at='2017-12-19 17:29:59'
    )
...
def test_dao_get_count_of_letters_to_process_for_today_without_notis_does_not_raise_error(
    sample_letter_template
):
    count_for_date = dao_get_count_of_letters_to_process_for_date()
...
@freeze_time("2017-12-18 17:50")
def test_dao_get_count_of_letters_to_process_for_date_ignores_research_mode_services(
    sample_letter_template
):
    research_service = create_service(
        service_name='research service', research_mode=True
    )
    ...
    # expected
    create_notification(
        template=sample_letter_template, created_at='2017-12-18 17:29:10'
    )
    create_notification(
        template=sample_letter_template, created_at='2017-12-18 17:29:20'
    )
...
@freeze_time("2017-12-18 17:50")
def test_dao_get_count_of_letters_to_process_for_date_ignores_test_keys(
    sample_letter_template
):
    # not expected
    create_notification(
        ...
        key_type=KEY_TYPE_TEST,
        created_at='2017-12-18 17:29:00',
    )
...
def test_notifications_not_yet_sent(sample_service, notification_type):
    ...
    create_notification(
        template=template, created_at=datetime.utcnow(), status='created'
    )
...
def test_notifications_not_yet_sent_return_no_rows(sample_service, notification_type):
    ...
    create_notification(
        template=template, created_at=datetime.utcnow(), status='sending'
    )
    create_notification(
        template=template, created_at=datetime.utcnow(), status='delivered'
    )
