...
def test_fetch_notification_status_totals_for_all_services_works_in_aet(
        notify_db_session
):
    service_1 = create_service(service_name='service_1')
    sms_template = create_template(service=service_1, template_type=SMS_TYPE)
...
@freeze_time('2018-03-30 12:59')  # In AET, March 30 2018 11:59pm
def test_fetch_monthly_template_usage_for_service(sample_service):
    ...
    create_ft_notification_status(aet_date=date(2017, 12, 10),
    ...
    create_ft_notification_status(aet_date=date(2018, 1, 1),
                                  service=sample_service,
                                  template=template_one,
                                  count=4)

    create_ft_notification_status(aet_date=date(2018, 3, 1),
                                  service=sample_service,
...
def test_fetch_monthly_template_usage_for_service_does_join_to_notifications_if_today_is_not_in_date_range(
    ...
    template_one = create_template(service=sample_service, template_type='sms', template_name='a')
    template_two = create_template(service=sample_service, template_type='email', template_name='b')
    create_ft_notification_status(aet_date=date(2018, 2, 1),
                                  service=template_two.service,
                                  template=template_two,
                                  count=15)
    create_ft_notification_status(aet_date=date(2018, 2, 2),
...
def test_get_total_sent_notifications_for_day_and_type_returns_right_notification_type(
    ...
    create_ft_notification_status(aet_date="2019-03-27", service=sample_template.service, template=sample_template,
                                  count=3)
    create_ft_notification_status(aet_date="2019-03-27", service=sample_email_template.service,
                                  template=sample_email_template, count=5)
    create_ft_notification_status(aet_date="2019-03-27", service=sample_letter_template.service,
...
def test_get_total_sent_notifications_for_day_and_type_returns_total_for_right_day(
    ...
    create_ft_notification_status(aet_date=date - timedelta(days=1), notification_type=sample_template.template_type,
                                  service=sample_template.service, template=sample_template, count=1)
    create_ft_notification_status(aet_date=date, notification_type=sample_template.template_type,
