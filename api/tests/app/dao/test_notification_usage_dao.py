...
from app.models import Rate, SMS_TYPE
...
def test_get_rates_for_daterange(notify_db, notify_db_session):
    ...
    assert len(rates) == 1
    assert (
        datetime.strftime(rates[0].valid_from, '%Y-%m-%d %H:%M:%S')
        == "2017-03-31 23:00:00"
    )
...
def test_get_rates_for_daterange_multiple_result_per_year(notify_db, notify_db_session):
    ...
    assert (
        datetime.strftime(rates[1].valid_from, '%Y-%m-%d %H:%M:%S')
        == "2016-05-18 00:00:00"
    )
...
def test_get_rates_for_daterange_returns_correct_rates(notify_db, notify_db_session):
    ...
    assert (
        datetime.strftime(rates_2017[0].valid_from, '%Y-%m-%d %H:%M:%S')
        == "2016-09-01 00:00:00"
    )
    assert rates_2017[0].rate == 0.016
    assert (
        datetime.strftime(rates_2017[1].valid_from, '%Y-%m-%d %H:%M:%S')
        == "2017-06-01 00:00:00"
    )
...
def test_get_monthly_billing_data(
...
def set_up_rate(notify_db, start_date, value):
    rate = Rate(
        id=uuid.uuid4(), valid_from=start_date, rate=value, notification_type=SMS_TYPE
    )
...
def test_get_billing_data_for_month_where_start_date_before_rate_returns_empty(
    ...
    results = get_monthly_billing_data(service_id=sample_template.service_id, year=2015)
