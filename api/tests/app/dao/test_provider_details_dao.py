...
def test_update_adds_history(restore_provider_details):
    ...
    smtp_history = (
        ProviderDetailsHistory.query.filter(ProviderDetailsHistory.id == smtp.id)
...
def test_switch_sms_provider_to_inactive_provider_does_not_switch(
    ...
    dao_switch_sms_provider_to_provider_with_identifier(
        alternative_sms_provider.identifier
    )
    new_provider = get_current_provider('sms')
...
def test_toggle_sms_provider_switches_provider(
    mocker, restore_provider_details, current_sms_provider, sample_user
):
    mocker.patch(
    ...
    old_starting_provider = get_provider_details_by_identifier(
        current_sms_provider.identifier
    )
...
def test_toggle_sms_provider_updates_provider_history(
    ...
    assert (
        updated_provider_history_rows[0].version - provider_history_rows[0].version == 1
    )


def test_toggle_sms_provider_switches_provider_stores_notify_user_id(
    restore_provider_details, sample_user, mocker
):
    mocker.patch(
        'app.provider_details.switch_providers.get_user_by_id', return_value=sample_user
    )
...
def test_toggle_sms_provider_switches_provider_stores_notify_user_id_in_history(
    ...
    old_provider_from_history = (
        ProviderDetailsHistory.query.filter_by(
            identifier=old_provider.identifier, version=old_provider.version
        )
    ...
    new_provider_from_history = (
        ProviderDetailsHistory.query.filter_by(
            identifier=new_provider.identifier, version=new_provider.version
        )
        .order_by(asc(ProviderDetailsHistory.priority))
        .first()
    )
