...
def test_should_have_decorated_notifications_dao_functions():
    assert (
        dao_get_notification_outcomes_for_job.__wrapped__.__name__
        == 'dao_get_notification_outcomes_for_job'
    )  # noqa
...
def test_should_count_of_statuses_for_notifications_associated_with_job(
    ...
    results = dao_get_notification_outcomes_for_job(sample_service.id, sample_job.id)
    assert set([(row.count, row.status) for row in results]) == set(
        [(2, 'created'), (4, 'sending'), (2, 'delivered')]
    )
...
def test_should_return_notifications_only_for_this_job(
...
def test_should_return_notifications_only_for_this_service(
    ...
    service_1 = create_service(
        notify_db, notify_db_session, service_name="one", email_from="one"
    )
    service_2 = create_service(
        notify_db, notify_db_session, service_name="two", email_from="two"
    )
    ...
    create_notification(
        notify_db, notify_db_session, service=service_1, job=job_1, status='created'
    )
    create_notification(
        notify_db, notify_db_session, service=service_2, job=job_2, status='created'
    )
...
def test_get_jobs_for_service(notify_db, notify_db_session, sample_template):
    ...
    other_template = create_template(
        notify_db, notify_db_session, service=other_service
    )
    other_job = create_job(
        notify_db, notify_db_session, service=other_service, template=other_template
    )
...
def test_set_scheduled_jobs_to_pending_gets_all_jobs_in_scheduled_state_before_now(
    ...
    job_old = create_job(
        notify_db, notify_db_session, scheduled_for=one_hour_ago, job_status='scheduled'
    )
...
@pytest.mark.parametrize('file_name', ['Test message', 'Report'])
...
def test_should_get_jobs_seven_days_old_filters_type(notify_db, notify_db_session):
    ...
    letter_template = create_template(
        notify_db, notify_db_session, template_type=LETTER_TYPE
    )
    sms_template = create_template(notify_db, notify_db_session, template_type=SMS_TYPE)
    email_template = create_template(
        notify_db, notify_db_session, template_type=EMAIL_TYPE
    )
...
def test_dao_get_letter_job_ids_by_status(sample_service):
    ...
    email_template = create_db_template(
        service=sample_service, template_type=EMAIL_TYPE
    )
    letter_template_1 = create_db_template(
        service=sample_service, template_type=LETTER_TYPE
    )
    letter_template_2 = create_db_template(
        service=another_service, template_type=LETTER_TYPE
    )
    letter_job_1 = create_db_job(
        letter_template_1,
        job_status=JOB_STATUS_READY_TO_SEND,
        original_file_name='1.csv',
    )
    letter_job_2 = create_db_job(
        letter_template_2,
        job_status=JOB_STATUS_READY_TO_SEND,
