...
def test_add_another_letter_contact_as_default_overrides_existing(notify_db_session):
    ...
    create_letter_contact(service=service, contact_block='Edinburgh, ED1 1AA')
    add_letter_contact_for_service(
        service_id=service.id, contact_block='Swansea, SN1 3CC', is_default=True
    )
...
def test_add_letter_contact_does_not_override_default(notify_db_session):
    ...
    add_letter_contact_for_service(
        service_id=service.id, contact_block='Edinburgh, ED1 1AA', is_default=True
    )
    add_letter_contact_for_service(
        service_id=service.id, contact_block='Swansea, SN1 3CC', is_default=False
    )
...
def test_can_update_letter_contact(notify_db_session):
    ...
    letter_contact = create_letter_contact(
        service=service, contact_block='Aberdeen, AB12 23X'
    )
...
def test_update_letter_contact_as_default_overides_existing_default(notify_db_session):
    ...
    create_letter_contact(service=service, contact_block='Aberdeen, AB12 23X')
    second_letter_contact = create_letter_contact(
        service=service, contact_block='Swansea, SN1 3CC', is_default=False
    )
...
def test_update_letter_contact_unset_default_for_only_letter_contact_raises_exception(
    ...
    service = create_service()
    only_letter_contact = create_letter_contact(
        service=service, contact_block='Aberdeen, AB12 23X'
    )
...
def test_dao_get_letter_contact_by_id(sample_service):
    letter_contact = create_letter_contact(
        service=sample_service, contact_block='Aberdeen, AB12 23X'
    )
    result = dao_get_letter_contact_by_id(
        service_id=sample_service.id, letter_contact_id=letter_contact.id
    )
...
def test_dao_get_letter_contact_by_id_raises_sqlalchemy_error_when_letter_contact_does_not_exist(
    ...
    with pytest.raises(SQLAlchemyError):
        dao_get_letter_contact_by_id(
            service_id=sample_service.id, letter_contact_id=uuid.uuid4()
        )
...
def test_dao_get_letter_contact_by_id_raises_sqlalchemy_error_when_service_does_not_exist(
    ...
    letter_contact = create_letter_contact(
        service=sample_service, contact_block='Some address'
    )
    with pytest.raises(SQLAlchemyError):
        dao_get_letter_contact_by_id(
            service_id=uuid.uuid4(), letter_contact_id=letter_contact.id
        )
