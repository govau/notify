...
def test_get_fragment_count_separates_sms_and_email(
    notify_db, sample_template, sample_email_template
):
...
def test_get_fragment_count_filters_on_service_id(
    notify_db, sample_template, service_factory
):
...
def test_get_fragment_count_filters_on_year(
    ...
    with freeze_time(creation_time):
        noti_hist(notify_db, sample_template)
    assert (
        get_fragment_count(sample_template.service_id, year=2000)['sms_count']
        == expected_count
    )
...
@pytest.mark.parametrize(
    'key_type,sms_count', [(KEY_TYPE_NORMAL, 1), (KEY_TYPE_TEAM, 1), (KEY_TYPE_TEST, 0)]
)
def test_get_fragment_count_ignores_test_api_keys(
    notify_db, sample_template, key_type, sms_count
):
