...
def test_should_have_decorated_services_dao_functions():
    assert (
        dao_fetch_monthly_historical_stats_for_service.__wrapped__.__name__
        == 'dao_fetch_monthly_historical_stats_for_service'
    )  # noqa
    assert (
        dao_fetch_todays_stats_for_service.__wrapped__.__name__
        == 'dao_fetch_todays_stats_for_service'
    )  # noqa
    assert (
        dao_fetch_stats_for_service.__wrapped__.__name__
        == 'dao_fetch_stats_for_service'
    )  # noqa
...
def test_cannot_create_service_with_no_user(notify_db_session, sample_user):
    ...
    assert "Can't flush None value found in collection Service.users" in str(
        excinfo.value
    )
...
def test_create_service_returns_service_with_default_permissions(service_factory):
    ...
    _assert_service_permissions(
        service.permissions, (SMS_TYPE, EMAIL_TYPE, LETTER_TYPE, INTERNATIONAL_SMS_TYPE)
    )


@pytest.mark.parametrize(
    "permission_to_remove, permission_remaining",
    [(SMS_TYPE, (EMAIL_TYPE, LETTER_TYPE)), (EMAIL_TYPE, (SMS_TYPE, LETTER_TYPE))],
)
def test_remove_permission_from_service_by_id_returns_service_with_correct_permissions(
    ...
    dao_remove_service_permission(
        service_id=sample_service.id, permission=permission_to_remove
    )

    service = dao_fetch_service_by_id(sample_service.id)
    _assert_service_permissions(
        service.permissions, (permission_remaining + (INTERNATIONAL_SMS_TYPE,))
    )
...
def test_removing_all_permission_returns_service_with_no_permissions(sample_service):
    ...
    dao_remove_service_permission(
        service_id=sample_service.id, permission=INTERNATIONAL_SMS_TYPE
    )
...
def test_remove_service_does_not_remove_service_permission_types(sample_service):
    ...
    assert set(p.name for p in ServicePermissionTypes.query.all()) == set(
        SERVICE_PERMISSION_TYPES
    )
...
def test_create_service_by_id_adding_and_removing_letter_returns_service_without_letter(
    ...
    _assert_service_permissions(
        service.permissions, (SMS_TYPE, EMAIL_TYPE, INTERNATIONAL_SMS_TYPE)
    )
...
def test_update_service_permission_creates_a_history_record_with_current_data(
    ...
    _assert_service_permissions(
        service.permissions, (SMS_TYPE, EMAIL_TYPE, INTERNATIONAL_SMS_TYPE, LETTER_TYPE)
    )
    ...
    assert (
        len(Service.get_history_model().query.filter_by(name='service_name').all()) == 3
    )
    assert (
        Service.get_history_model()
...
def test_add_existing_user_to_another_service_doesnot_change_old_permissions(
    ...
    test_user_permissions = Permission.query.filter_by(
        service=service_one, user=sample_user
    ).all()
...
def test_fetch_stats_counts_correctly(
    ...
    create_notification(
        notify_db, notify_db_session, template=sample_email_template, status='created'
    )
    create_notification(
        notify_db, notify_db_session, template=sample_email_template, status='created'
    )
    ...
    create_notification(
        notify_db, notify_db_session, template=sample_template, status='created'
    )
...
def test_fetch_monthly_historical_stats_separates_months(
    notify_db, notify_db_session, sample_template
):
    notification_history = functools.partial(
        create_notification_history, notify_db, notify_db_session, sample_template
    )
    ...
    notification_history(
        created_at=datetime(2016, 9, 30, 23, 30, 0)
    )  # October because BST
    # start_of_winter
    ...
    result = dao_fetch_monthly_historical_stats_for_service(
        sample_template.service_id, 2016
    )
...
def test_dao_fetch_todays_stats_for_all_services_includes_all_services(
    notify_db, notify_db_session, service_factory
):
...
def test_fetch_stats_by_date_range_for_all_services(notify_db, notify_db_session):
    create_notification(
        notify_db, notify_db_session, created_at=datetime.now() - timedelta(days=4)
    )
    create_notification(
        notify_db, notify_db_session, created_at=datetime.now() - timedelta(days=3)
    )
    result_one = create_notification(
        notify_db, notify_db_session, created_at=datetime.now() - timedelta(days=2)
    )
    create_notification(
        notify_db, notify_db_session, created_at=datetime.now() - timedelta(days=1)
    )
    ...
    assert results[0] == (
        result_one.service.id,
        result_one.service.name,
        result_one.service.restricted,
        result_one.service.research_mode,
        result_one.service.active,
        result_one.service.created_at,
        'sms',
        'created',
        2,
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "1",
            "4",
        ),  # a date range less than 7 days ago returns test and normal notifications
        (
            "9",
            "8",
            "1",
        ),  # a date range older than 9 days does not return test notifications.
        ("8", "4", "2"),
    ],
)  # a date range that starts more than 7 days ago
@freeze_time('2017-10-23T00:00:00')
def test_fetch_stats_by_date_range_for_all_services_returns_test_notifications(
    ...
    results = fetch_stats_by_date_range_for_all_services(
...
def test_dao_allocating_inbound_number_shows_on_service(
...
def test_dao_fetch_monthly_historical_stats_by_template(notify_db, notify_db_session):
    ...
    template_two = create_sample_template(
        notify_db, notify_db_session, template_name='2'
    )
...
def test_dao_fetch_monthly_historical_usage_by_template_for_service_no_stats_today(
    ...
    n = notification_history(
        created_at=datetime(2017, 10, 1), sample_template=template_one
    )
...
def test_dao_fetch_monthly_historical_usage_by_template_for_service_add_to_historical(
    ...
    template_three = create_sample_template(
        notify_db, notify_db_session, template_name='3'
    )
    ...
    n = notification_history(
        created_at=datetime(2017, 9, 1), sample_template=template_one
    )
    notification_history(
        created_at=datetime(year, month, day) - timedelta(days=1),
        sample_template=template_two,
    )
    notification_history(
        created_at=datetime(year, month, day) - timedelta(days=1),
        sample_template=template_two,
...
def test_dao_fetch_monthly_historical_usage_by_template_for_service_does_add_old_notification(
    ...
    template_one = create_sample_template(
        notify_db, notify_db_session, template_name='1', template_type='email'
    )
    template_two = create_sample_template(
        notify_db, notify_db_session, template_name='2', template_type='sms'
    )
    template_three = create_sample_template(
        notify_db, notify_db_session, template_name='3', template_type='letter'
    )
...
def test_dao_fetch_monthly_historical_usage_by_template_for_service_get_this_year_only(
    ...
    n = notification_history(
        created_at=datetime(2016, 9, 1), sample_template=template_one
    )
    notification_history(
        created_at=datetime(year, month, day) - timedelta(days=1),
        sample_template=template_two,
...
def test_dao_fetch_monthly_historical_usage_by_template_for_service_combined_historical_current(
    ...
    n = notification_history(
        created_at=datetime(year, month, day) - timedelta(days=30),
...
def test_dao_fetch_monthly_historical_usage_by_template_for_service_ignores_test_api_keys(
    sample_service
):
