...
def test_should_build_service_whitelist_from_mobile_number(mobile_number):
    service_whitelist = ServiceWhitelist.from_string(
...
@pytest.mark.parametrize(
    'template_type, recipient', [('sms', '+447700900855'), ('email', 'foo@bar.com')]
)
def test_notification_for_csv_returns_correct_type(
    notify_db, notify_db_session, template_type, recipient
):
    template = create_sample_template(
        notify_db, notify_db_session, template_type=template_type
    )
    notification = create_sample_notification_with_job(
        notify_db, notify_db_session, template=template, to_field=recipient
    )
...
def test_notification_for_csv_returns_correct_job_row_number(
    ...
    notification = create_sample_notification_with_job(
        notify_db, notify_db_session, job_row_number=0
    )
...
def test_notification_for_csv_returns_formatted_status(
    notify_db, notify_db_session, template_type, status, expected_status
):
    template = create_sample_template(
        notify_db, notify_db_session, template_type=template_type
    )
...
def test_notification_for_csv_returns_bst_correctly(notify_db, notify_db_session):
    notification = create_sample_notification_with_job(
        notify_db, notify_db_session, job_row_number=100, status='permanent-failure'
    )
...
def test_notification_subject_fills_in_placeholders_for_email(
    sample_email_template_with_placeholders
):
    noti = create_notification(
        sample_email_template_with_placeholders, personalisation={'name': 'hello'}
    )
...
def test_notification_subject_fills_in_placeholders_for_letter(sample_letter_template):
    noti = create_notification(
        sample_letter_template, personalisation={'name': 'hello'}
    )
    noti.template.subject = '((name))'
    assert noti.subject == 'hello'


def test_letter_notification_serializes_with_address(
...
def test_letter_notification_postcode_can_be_null_for_precompiled_letters(
    client, sample_letter_notification
):
    sample_letter_notification.personalisation = {
        'address_line_1': 'test',
