...
def test_get_service_by_id_returns_organisation_type(
    admin_request, sample_service, detailed
):
    json_resp = admin_request.get(
        'service.get_service_by_id', service_id=sample_service.id, detailed=detailed
    )
...
def test_get_service_list_has_default_permissions(admin_request, service_factory):
    ...
    assert all(
        ...
        == set([EMAIL_TYPE, SMS_TYPE, INTERNATIONAL_SMS_TYPE, LETTER_TYPE])
...
def test_get_service_by_id_should_404_if_no_service(admin_request, notify_db_session):
    json_resp = admin_request.get(
        'service.get_service_by_id', service_id=uuid.uuid4(), _expected_status=404
    )
...
def test_update_service(client, notify_db, sample_service):
    brand = EmailBranding(
        colour='#000000', logo='justice-league.png', name='Justice League'
    )
...
def test_update_service_flags(client, sample_service):
    ...
    assert set(result['data']['permissions']) == set(
        [LETTER_TYPE, INTERNATIONAL_SMS_TYPE]
    )


@pytest.mark.parametrize(
    "org_type, expected", [("central", True), ('local', False), ("nhs", False)]
)
...
def test_update_permissions_with_duplicate_permissions_will_raise_error(
    ...
    assert result['result'] == 'error'
    assert (
        "Duplicate Service Permission: ['{}']".format(LETTER_TYPE)
        in result['message']['permissions']
    )
...
def test_add_existing_user_to_non_existing_service_returns404(
    notify_api, notify_db, notify_db_session, sample_user
):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            user_to_add = User(
...
def test_remove_user_from_service(
    ...
    second_permission = create_user_service_permission(
        notify_db, notify_db_session, user=second_user
    )
    endpoint = url_for(
...
def test_cannot_remove_only_user_from_service(
    notify_api, notify_db, notify_db_session, sample_user_service_permission
):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            endpoint = url_for(
...
def test_get_all_notifications_for_service_in_order(
    notify_api, notify_db, notify_db_session
):
    with notify_api.test_request_context(), notify_api.test_client() as client:
        ...
        notification_1 = create_sample_notification(
            notify_db, notify_db_session, service=service_1
        )
...
def test_get_all_notifications_for_service_including_ones_made_by_jobs(
    ...
    with_job = sample_notification_with_job(
        notify_db, notify_db_session, service=sample_service
    )
    without_job = create_sample_notification(
        notify_db, notify_db_session, service=sample_service
    )
    # from_test_api_key
...
def test_get_only_api_created_notifications_for_service(
    admin_request, sample_job, sample_template
):
...
@pytest.mark.parametrize('should_prefix', [True, False])
def test_prefixing_messages_based_on_prefix_sms(
    client, notify_db_session, should_prefix
):
...
def test_set_sms_prefixing_for_service(
    admin_request, client, sample_service, posted_value, stored_value, returned_value
):
...
def test_set_sms_prefixing_for_service_cant_be_none(admin_request, sample_service):
    resp = admin_request.post(
        'service.update_service',
        service_id=sample_service.id,
...
def test_get_detailed_service(
    notify_db, notify_db_session, notify_api, sample_service, today_only, stats
):
    with notify_api.test_request_context(), notify_api.test_client() as client:
...
@pytest.mark.parametrize(
    ...
    [
        ('/service/{}/notifications/monthly?year=2001', 200, {'data': {'foo': 'bar'}}),
        (
...
def test_get_monthly_notification_stats(
    mocker, client, sample_service, url, expected_status, expected_json
):
    mocker.patch(
    ...
    response = client.get(
        url.format(sample_service.id), headers=[create_authorization_header()]
    )
...
def test_get_services_with_detailed_flag(client, notify_db, notify_db_session):
    notifications = [
        ...
        create_sample_notification(notify_db, notify_db_session),
        create_sample_notification(
            notify_db, notify_db_session, key_type=KEY_TYPE_TEST
        ),
    ]
...
def test_get_detailed_services_groups_by_service(notify_db, notify_db_session):
    ...
    create_sample_notification(
        notify_db, notify_db_session, service=service_1, status='created'
    )
    create_sample_notification(
        notify_db, notify_db_session, service=service_2, status='created'
    )
    create_sample_notification(
        notify_db, notify_db_session, service=service_1, status='delivered'
    )
    create_sample_notification(
        notify_db, notify_db_session, service=service_1, status='created'
    )
...
def test_get_detailed_services_only_includes_todays_notifications(
    ...
    create_sample_notification(
        notify_db, notify_db_session, created_at=datetime(2015, 10, 9, 23, 59)
    )
    create_sample_notification(
        notify_db, notify_db_session, created_at=datetime(2015, 10, 10, 0, 0)
    )
    create_sample_notification(
        notify_db, notify_db_session, created_at=datetime(2015, 10, 10, 12, 0)
    )
    create_sample_notification(
        notify_db, notify_db_session, created_at=datetime(2015, 10, 10, 23, 0)
    )
...
def test_get_detailed_services_for_date_range(notify_db, notify_db_session, set_time):
    ...
    with freeze_time(set_time):
        ...
        create_sample_notification(
            notify_db, notify_db_session, created_at=datetime.utcnow()
        )
...
@pytest.mark.parametrize(
    'query_string, expected_status, expected_json',
    [
...
def test_get_service_provider_aggregate_statistics(
    client, sample_service, query_string, expected_status, expected_json
):
    response = client.get(
        '/service/{}/fragment/aggregate_statistics{}'.format(
            sample_service.id, query_string
        ),
        headers=[create_authorization_header()],
    )
    assert response.status_code == expected_status
...
def test_get_template_usage_by_month_returns_correct_data(
    ...
    not1 = create_notification_history(
        notify_db, notify_db_session, sample_template, created_at=datetime(2016, 4, 1)
    )
    ...
    resp = client.get(
        '/service/{}/notifications/templates_usage/monthly?year=2017'.format(
            not1.service_id
        ),
        headers=[create_authorization_header()],
    )
...
def test_get_template_usage_by_month_returns_no_data(
    ...
    resp = client.get(
        '/service/{}/notifications/templates_usage/monthly?year=2015'.format(
            not1.service_id
        ),
        headers=[create_authorization_header()],
    )
...
def test_get_template_usage_by_month_returns_two_templates(
    ...
    # add a historical notification for template
    not1 = create_notification_history(
        notify_db, notify_db_session, template_one, created_at=datetime(2017, 4, 1)
    )
    ...
    resp_json = sorted(
        resp_json,
        key=lambda k: (k.get('year', 0), k.get('month', 0), k.get('count', 0)),
    )
...
def test_search_for_notification_by_to_field_return_empty_list_if_there_is_no_match(
    ...
    response = client.get(
        '/service/{}/notifications?to={}&template_type={}'.format(
            notification1.service_id, '+447700900800', 'sms'
        ),
        headers=[create_authorization_header()],
    )
    notifications = json.loads(response.get_data(as_text=True))['notifications']
...
def test_search_for_notification_by_to_field_return_multiple_matches(
    ...
    notification1 = create_notification(
        to_field='+447700900855', normalised_to='447700900855'
    )
    notification2 = create_notification(
        to_field=' +44 77009 00855 ', normalised_to='447700900855'
    )
    notification3 = create_notification(
        to_field='+44770 0900 855', normalised_to='447700900855'
    )
    notification4 = create_notification(
        to_field='jack@gmail.com', normalised_to='jack@gmail.com'
    )

    response = client.get(
        '/service/{}/notifications?to={}&template_type={}'.format(
            notification1.service_id, '+447700900855', 'sms'
        ),
        headers=[create_authorization_header()],
    )
    notifications = json.loads(response.get_data(as_text=True))['notifications']
...
def test_search_for_notification_by_to_field_return_400_for_letter_type(
    ...
    response = client.get(
        '/service/{}/notifications?to={}&template_type={}'.format(
            sample_service.id, 'A. Name', 'letter'
        ),
        headers=[create_authorization_header()],
    )
...
def test_search_for_notification_by_to_field_filters_by_status(
    client, notify_db, notify_db_session
):
    create_notification = partial(
        create_sample_notification,
...
def test_get_notification_for_service_includes_precompiled_letter(
    admin_request, sample_notification
):
    resp = admin_request.get(
        'service.get_notification_for_service',
...
def test_is_service_name_unique_returns_200_if_unique(
...
@pytest.mark.parametrize(
    'name, email_from', [("existing name", "email.from"), ("name", "existing.name")]
)
...
def test_add_service_reply_to_email_address_404s_when_invalid_service_id(
    ...
    response = client.post(
        '/service/{}/email-reply-to'.format(uuid.uuid4()),
        data={},
        headers=[('Content-Type', 'application/json'), create_authorization_header()],
    )
...
def test_update_service_reply_to_email_address(client, sample_service):
    ...
    response = client.post(
        '/service/{}/email-reply-to/{}'.format(sample_service.id, original_reply_to.id),
        data=data,
        headers=[('Content-Type', 'application/json'), create_authorization_header()],
    )
...
def test_update_service_reply_to_email_address_404s_when_invalid_service_id(
    ...
    response = client.post(
        '/service/{}/email-reply-to/{}'.format(uuid.uuid4(), uuid.uuid4()),
        data={},
        headers=[('Content-Type', 'application/json'), create_authorization_header()],
    )
...
def test_get_letter_contacts_with_one_letter_contact(
    client, notify_db, notify_db_session
):
    service = create_service()
...
def test_get_letter_contacts_with_multiple_letter_contacts(
    client, notify_db, notify_db_session
):
    service = create_service()
...
def test_get_letter_contact_return_404_when_invalid_contact_id(
    client, notify_db, notify_db_session
):
    service = create_service()
    ...
    response = client.get(
        '/service/{}/letter-contact/{}'.format(
            service.id, '93d59f88-4aa1-453c-9900-f61e2fc8a2de'
        ),
        headers=[('Content-Type', 'application/json'), create_authorization_header()],
    )
...
def test_add_service_letter_contact_block_raise_exception_if_no_default(
    client, sample_service
):
...
def test_add_service_letter_contact_block_404s_when_invalid_service_id(
    client, notify_db, notify_db_session
):
    response = client.post(
...
def test_update_service_letter_contact(client, sample_service):
    original_letter_contact = create_letter_contact(
        service=sample_service, contact_block="Aberdeen, AB23 1XH"
    )
    data = json.dumps({"contact_block": "London, E1 8QS", "is_default": True})
    response = client.post(
        '/service/{}/letter-contact/{}'.format(
            sample_service.id, original_letter_contact.id
        ),
        data=data,
        headers=[('Content-Type', 'application/json'), create_authorization_header()],
    )
...
def test_update_service_letter_contact_returns_400_when_no_default(
    client, sample_service
):
    original_reply_to = create_letter_contact(
        service=sample_service, contact_block="Aberdeen, AB23 1XH"
    )
    data = json.dumps({"contact_block": "London, E1 8QS", "is_default": False})
    response = client.post(
...
def test_update_service_letter_contact_returns_404_when_invalid_service_id(
    client, notify_db, notify_db_session
):
    response = client.post(
...
def test_add_service_sms_sender_can_add_multiple_senders(client, notify_db_session):
    service = create_service()
    data = {"sms_sender": 'second', "is_default": False}
    response = client.post(
...
def test_add_service_sms_sender_switches_default(client, notify_db_session):
    ...
    data = {"sms_sender": 'second', "is_default": True}
    response = client.post(
        '/service/{}/sms-sender'.format(service.id),
        data=json.dumps(data),
        headers=[('Content-Type', 'application/json'), create_authorization_header()],
...
def test_get_service_sms_sender_by_id(client, notify_db_session):
    ...
    response = client.get(
        '/service/{}/sms-sender/{}'.format(
            service_sms_sender.service_id, service_sms_sender.id
        ),
        headers=[('Content-Type', 'application/json'), create_authorization_header()],
    )
    assert response.status_code == 200
...
def test_get_organisation_for_service_id(
    admin_request, sample_service, sample_organisation
):
    dao_add_service_to_organisation(sample_service, sample_organisation.id)
    response = admin_request.get(
        'service.get_organisation_for_service', service_id=sample_service.id
    )
    assert response == sample_organisation.serialize()


def test_get_organisation_for_service_id_return_empty_dict_if_service_not_in_organisation(
    admin_request, fake_uuid
):
    response = admin_request.get(
        'service.get_organisation_for_service', service_id=fake_uuid
    )
    assert response == {}
