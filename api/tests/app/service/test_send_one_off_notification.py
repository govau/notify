...
def test_send_one_off_notification_calls_celery_correctly(
    ...
    celery_mock.assert_called_once_with(
        notification=persist_mock.return_value, research_mode=False, queue=None
    )
...
def test_send_one_off_notification_raises_if_message_too_long(
    ...
    assert (
        e.value.message
        == 'Content for template has a character count greater than the limit of 495'
    )
...
def test_send_one_off_notification_should_add_email_reply_to_text_for_notification(
    ...
    celery_mock.assert_called_once_with(
        notification=notification, research_mode=False, queue=None
    )
...
def test_send_one_off_letter_notification_should_use_template_reply_to_text(
    ...
    letter_contact = create_letter_contact(
        sample_letter_template.service, "Edinburgh, ED1 1AA", is_default=False
    )
...
def test_send_one_off_sms_notification_should_use_sms_sender_reply_to_text(
    ...
    sms_sender = create_service_sms_sender(
        service=sample_service, sms_sender='07123123123', is_default=False
    )
...
def test_send_one_off_sms_notification_should_use_default_service_reply_to_text(
    ...
    create_service_sms_sender(
        service=sample_service, sms_sender='07123123456', is_default=True
    )
