...
def test_archive_only_allows_post(client):
    ...
    response = client.get(
        '/service/{}/archive'.format(uuid.uuid4()), headers=[auth_header]
    )
...
def test_deactivating_inactive_service_does_nothing(client, sample_service):
    ...
    response = client.post(
        '/service/{}/archive'.format(sample_service.id), headers=[auth_header]
    )
...
@pytest.fixture
def archived_service_with_deleted_stuff(
...
def test_deactivating_service_doesnt_affect_existing_archived_templates(
    ...
    assert archived_service_with_deleted_stuff.templates[0].updated_at == datetime(
        2001, 1, 1, 0, 0, 0
    )
...
def test_deactivating_service_doesnt_affect_existing_revoked_api_keys(
    ...
    assert archived_service_with_deleted_stuff.api_keys[0].expiry_date == datetime(
        2001, 1, 1, 0, 0, 0
    )
    assert archived_service_with_deleted_stuff.api_keys[0].version == 2


def test_deactivating_service_rolls_back_everything_on_error(
    sample_service, sample_api_key, sample_template
):
