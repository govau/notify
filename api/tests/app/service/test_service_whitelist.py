...
from app.models import ServiceWhitelist, MOBILE_TYPE, EMAIL_TYPE
...
def test_get_whitelist_returns_data(client, sample_service_whitelist):
    ...
    response = client.get(
        'service/{}/whitelist'.format(service_id),
...
def test_get_whitelist_separates_emails_and_phones(client, sample_service):
    dao_add_and_commit_whitelisted_contacts(
        [
            ...
            ServiceWhitelist.from_string(
                sample_service.id, MOBILE_TYPE, '+1800-555-555'
            ),
        ]
    )

    response = client.get(
        'service/{}/whitelist'.format(sample_service.id),
        headers=[create_authorization_header()],
    )
    assert response.status_code == 200
...
def test_update_whitelist_doesnt_remove_old_whitelist_if_error(
    client, sample_service_whitelist
):
