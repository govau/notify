...
def test_populate_redis_template_usage_only_populates_for_today(
    ...
    create_notification(
        sample_template, created_at=datetime(2017, 6, 10, 23, 0, 0)
    )  # actually on 11th BST
...
@freeze_time('2017-06-12')
def test_populate_redis_template_usage_only_populates_for_given_service(
    mocker, notify_api, notify_db_session
):
    mock_redis = mocker.patch('app.commands.redis_store')
    # created at in utc
