...
def letter_request(
    ...
    if precompiled:
        ...
        url = url_for(
            'v2_notifications.post_notification', notification_type=LETTER_TYPE
        )
    resp = client.post(
        url,
...
def test_post_letter_notification_returns_201(
    client, sample_letter_template, mocker, reference
):
    mock = mocker.patch(
        'app.celery.tasks.letters_pdf_tasks.create_letters_pdf.apply_async'
    )
    data = {
    ...
    resp_json = letter_request(
        client, data, service_id=sample_letter_template.service_id
    )
...
def test_notification_returns_400_if_address_doesnt_have_underscores(
    ...
    error_json = letter_request(
        client, data, service_id=sample_letter_template.service_id, _expected_status=400
    )
...
def test_returns_a_429_limit_exceeded_if_rate_limit_exceeded(
    ...
    error_json = letter_request(
        client, data, service_id=sample_letter_template.service_id, _expected_status=429
    )
...
def test_post_letter_notification_returns_403_if_not_allowed_to_send_notification(
    ...
    error_json = letter_request(
        client, data, service_id=service.id, _expected_status=400
    )
    assert error_json['status_code'] == 400
    assert error_json['errors'] == [
...
def test_post_letter_notification_doesnt_send_in_trial(
    ...
    error_json = letter_request(
        client, data, sample_trial_letter_template.service_id, _expected_status=403
    )
...
def test_post_letter_notification_is_delivered_and_has_pdf_uploaded_to_test_letters_bucket_using_test_key(
    ...
    sample_letter_service = create_service(
        service_permissions=['letter', 'precompiled_letter']
    )
    s3mock = mocker.patch(
    ...
    mocker.patch(
        'app.v2.notifications.post_notifications.pdf_page_count', return_value=1
    )
...
def test_post_letter_notification_persists_notification_reply_to_text(
    ...
    letter_contact = create_letter_contact(
        service=service, contact_block=service_address, is_default=True
    )
    template = create_template(
        service=service, template_type='letter', reply_to=letter_contact.id
    )
    data = {
...
def test_post_precompiled_letter_requires_permission(
    client, sample_service, notify_user, mocker
):
...
def test_post_precompiled_letter_with_invalid_base64(client, notify_user, mocker):
    ...
    assert (
        ...
        == 'Cannot decode letter content (invalid base64 encoding)'
    )
...
def test_post_precompiled_letter_notification_returns_201(client, notify_user, mocker):
    ...
    mocker.patch(
        'app.v2.notifications.post_notifications.pdf_page_count', return_value=5
    )
...
def test_post_precompiled_letter_notification_returns_400_with_invalid_pdf(
    client, notify_user, mocker
):
    sample_service = create_service(
        service_permissions=['letter', 'precompiled_letter']
