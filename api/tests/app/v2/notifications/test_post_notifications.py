...
def test_post_sms_notification_uses_sms_sender_id_reply_to(
    ...
    sms_sender = create_service_sms_sender(
        service=sample_template_with_placeholders.service, sms_sender='0412345679'
    )
    mocked = mocker.patch('app.celery.provider_tasks.deliver_sms.apply_async')
    data = {
        'phone_number': '+61412345678',
...
def test_post_email_notification_returns_201(
    ...
    assert resp_json['content'][
        'body'
    ] == sample_email_template_with_placeholders.content.replace(
        '((name))', 'Bob'
    ).replace(
        'GOV.AU', u'GOV.\u200bAU\n'
    )
...
@pytest.mark.parametrize(
    'recipient,label,template_factory,expected_error',
    [
        (
            '07700 900000',
            'phone_number',
...
def test_post_sms_notification_returns_201_if_allowed_to_send_int_sms(
    sample_service, sample_template, client, mocker
):
    mocker.patch('app.celery.provider_tasks.deliver_sms.apply_async')
...
def test_post_notification_with_scheduled_for(
    client, notify_db, notify_db_session, notification_type, key_send_to, send_to
):
