...
def test_get_notifications_request_invalid_statuses(invalid_statuses, valid_statuses):
    partial_error_status = "is not one of " "[created, sending, sent, delivered, pending, failed, " "technical-failure, temporary-failure, permanent-failure, pending-virus-check, " "virus-scan-failed, accepted, received]"
...
def test_get_notifications_request_invalid_statuses_and_template_types():
    ...
    for invalid_status in ["elephant", "giraffe"]:
        assert (
            "status {} is not one of [created, sending, sent, delivered, "
    ...
    for invalid_template_type in ["orange", "avocado"]:
        assert (
            "template_type {} is not one of [sms, email, letter]".format(
...
def test_post_sms_request_schema_invalid_phone_number_and_missing_template():
    ...
    assert {
        "error": "ValidationError",
        "message": "phone_number Not an AU mobile number",
    } in errors
    assert {
        "error": "ValidationError",
