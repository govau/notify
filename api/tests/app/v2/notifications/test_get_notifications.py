...
from tests.app.conftest import sample_notification, sample_email_notification


@pytest.mark.parametrize(
    'billable_units, provider', [(1, 'mmg'), (0, 'mmg'), (1, None)]
)
...
def test_get_notifications_returns_scheduled_for(client, sample_template):
    ...
    assert (
        json_response['notifications'][0]['scheduled_for']
        == "2017-05-23T16:15:00.000000Z"
    )
...
@pytest.mark.parametrize(
    'created_at_month, estimated_delivery',
    [
...
def test_get_notification_adds_delivery_estimate_for_letters(
    client, sample_letter_notification, created_at_month, estimated_delivery
):
...
@pytest.mark.parametrize(
    'notification_mock', [sample_notification, sample_email_notification]
)
def test_get_notification_doesnt_have_delivery_estimate_for_non_letters(
    client, notify_db, notify_db_session, notification_mock
):
...
def test_get_all_notifications_filter_by_status_invalid_status(
    ...
    assert (
        json_response['errors'][0]['message']
        == "status elephant is not one of [created, sending, sent, "
...
def test_get_all_notifications_filter_multiple_query_parameters(
    ...
    auth_header = create_authorization_header(service_id=newer_notification.service_id)
    response = client.get(
        path='/v2/notifications?status=pending&template_type=email&older_than={}'.format(
...
def test_get_all_notifications_renames_letter_statuses(
    client, sample_letter_notification, sample_notification, sample_email_notification
):
    auth_header = create_authorization_header(
        service_id=sample_letter_notification.service_id
    )
...
def test_get_notifications_renames_letter_statuses(
    ...
    response = client.get(
        path=url_for(
            'v2_notifications.get_notification_by_id', notification_id=letter_noti.id
        ),
        headers=[('Content-Type', 'application/json'), auth_header],
    )
