...
def test_should_not_allow_invalid_secret(client, sample_api_key):
    ...
    response = client.get(
        '/notifications', headers={'Authorization': "Bearer {}".format(token)}
    )
...
def test_should_allow_valid_token(client, sample_api_key, scheme):
    ...
    response = client.get(
        '/notifications', headers={'Authorization': '{} {}'.format(scheme, token)}
    )
...
def test_should_not_allow_service_id_that_is_not_the_wrong_data_type(
    ...
    data = json.loads(response.get_data())
    assert data['message'] == {
        "token": ['Invalid token: service id is not the right data type']
    }
...
def test_authentication_passes_when_service_has_multiple_keys_some_expired(
    ...
    save_model_api_key(api_key)
    token = create_jwt_token(
        secret=get_unsigned_secret(api_key.id), client_id=str(sample_api_key.service_id)
    )
    response = client.get(
        '/notifications', headers={'Authorization': 'Bearer {}'.format(token)}
    )
...
def test_authentication_returns_error_when_admin_client_secret_is_invalid(client):
    api_secret = current_app.config.get('ADMIN_CLIENT_SECRET')
    token = create_jwt_token(
        secret=api_secret, client_id=current_app.config.get('ADMIN_CLIENT_USER_NAME')
    )
...
def test_authentication_returns_error_when_service_doesnt_exit(client, sample_api_key):
    ...
    token = create_jwt_token(
        secret=str(sample_api_key.service_id), client_id=str(sample_api_key.id)
    )
...
def test_authentication_returns_error_when_service_inactive(client, sample_api_key):
    ...
    token = create_jwt_token(
        secret=str(sample_api_key.id), client_id=str(sample_api_key.service_id)
    )
...
def __create_token(service_id):
    return create_jwt_token(
        secret=get_unsigned_secrets(service_id)[0], client_id=str(service_id)
    )
...
def test_proxy_key_non_auth_endpoint(
    notify_api, check_proxy_header, header_value, expected_status
):
    with set_config_values(
        ...
        with notify_api.test_client() as client:
            response = client.get(
                path='/_status', headers=[('X-Custom-Forwarder', header_value)]
            )
        assert response.status_code == expected_status


@pytest.mark.skip(
    reason="Our architecture does not include an API Gateway for caching/rate limiting. We may include this in the future."
)
@pytest.mark.parametrize(
    'check_proxy_header,header_value,expected_status',
    [
...
def test_proxy_key_on_admin_auth_endpoint(
    notify_api, check_proxy_header, header_value, expected_status
):
    token = create_jwt_token(
        current_app.config['ADMIN_CLIENT_SECRET'],
