...
def test_send_processing_time_to_performance_platform_generates_correct_calls(
    ...
    create_notification(
        sample_template, created_at=datetime.utcnow() - timedelta(days=2)
    )
    ...
    send_mock.assert_any_call(
        datetime(2016, 10, 16, 23, 0), 'messages-within-10-secs', 1
    )
...
def test_send_processing_time_to_performance_platform_creates_correct_call_to_perf_platform(
    ...
    send_processing_time_data(
        date=datetime(2016, 10, 15, 23, 0, 0), status='foo', count=142
    )
    ...
    expected_base64_id = (
        'MjAxNi0xMC0xNlQwMDowMDowMGdvdnVrLW5vdGlmeWZvb3Byb2Nlc3NpbmctdGltZWRheQ=='
    )
