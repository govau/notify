    ...
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            with freeze_time("2016-01-01 12:00:00.000000"):
                scheduled_date = (
                    datetime.utcnow() + timedelta(hours=95, minutes=59)
                ).isoformat()
                mocker.patch('app.celery.tasks.process_job.apply_async')
                data = {
                    'id': fake_uuid,
...
def test_create_job_returns_403_if_letter_template_type_and_service_in_trial(
    ...
    assert (
        resp_json['message']
        == "Create letter job is not allowed for service in trial mode "
    )
    mock_job_dao.assert_not_called()


def test_should_not_create_scheduled_job_more_then_24_hours_hence(
    notify_api, sample_template, mocker, fake_uuid
):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            with freeze_time("2016-01-01 11:09:00.061258"):
                scheduled_date = (
                    datetime.utcnow() + timedelta(hours=96, minutes=1)
                ).isoformat()
    ...
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            ...
            assert (
                ...
                in resp_json['message']['original_file_name']
            )
            assert (
                'Missing data for required field.'
                in resp_json['message']['notification_count']
            )
...
def test_create_job_returns_404_if_template_does_not_exist(
    ...
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            mocker.patch('app.celery.tasks.process_job.apply_async')
            data = {'template': str(sample_service.id)}
            path = '/service/{}/job'.format(sample_service.id)
            auth_header = create_authorization_header()
            headers = [('Content-Type', 'application/json'), auth_header]
            response = client.post(path, data=json.dumps(data), headers=headers)
...
def _setup_jobs(notify_db, notify_db_session, template, number_of_jobs=5):
    for i in range(number_of_jobs):
        create_job(
            notify_db, notify_db_session, service=template.service, template=template
        )
...
def test_get_all_notifications_for_job_in_order_of_job_number(
    ...
    with notify_api.test_request_context(), notify_api.test_client() as client:
        ...
        response = client.get(
            path='/service/{}/job/{}/notifications'.format(
        ...
        assert (
            resp['notifications'][0]['job_row_number'] == notification_1.job_row_number
        )
        assert resp['notifications'][1]['to'] == notification_2.to
        assert (
            resp['notifications'][1]['job_row_number'] == notification_2.job_row_number
        )
        assert resp['notifications'][2]['to'] == notification_3.to
        assert (
            resp['notifications'][2]['job_row_number'] == notification_3.job_row_number
        )
        assert response.status_code == 200
...
def test_get_all_notifications_for_job_returns_correct_format(
    client, sample_notification_with_job
):
...
def test_get_job_by_id_should_return_statistics(
    notify_db, notify_db_session, notify_api, sample_job
):
    job_id = str(sample_job.id)
    service_id = sample_job.service.id
    ...
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            ...
            assert {'status': 'delivered', 'count': 1} in resp_json['data'][
                'statistics'
            ]
            ...
            assert {'status': 'temporary-failure', 'count': 1} in resp_json['data'][
                'statistics'
            ]
            assert {'status': 'permanent-failure', 'count': 1} in resp_json['data'][
                'statistics'
            ]
            assert resp_json['data']['created_by']['name'] == 'Test User'


def test_get_job_by_id_should_return_summed_statistics(
    notify_db, notify_db_session, notify_api, sample_job
):
    job_id = str(sample_job.id)
    service_id = sample_job.service.id
...
def test_get_jobs_with_limit_days(
    notify_api, notify_db, notify_db_session, sample_template
):
    create_job(
        notify_db,
        notify_db_session,
    ...
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            ...
            auth_header = create_authorization_header()
            response = client.get(
                path, headers=[auth_header], query_string={'limit_days': 5}
            )
            assert response.status_code == 200
            resp_json = json.loads(response.get_data(as_text=True))
...
def test_get_jobs_should_return_statistics(
    ...
    job_1 = create_job(
        notify_db, notify_db_session, service=sample_service, created_at=earlier
    )
    job_2 = create_job(
        notify_db, notify_db_session, service=sample_service, created_at=now
    )
    ...
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            ...
            assert {'status': 'sending', 'count': 3} in resp_json['data'][0][
                'statistics'
            ]
            assert resp_json['data'][1]['id'] == str(job_1.id)
            assert {'status': 'created', 'count': 3} in resp_json['data'][1][
                'statistics'
            ]
...
def test_get_jobs_should_paginate(
    ...
    create_10_jobs(
        notify_db, notify_db_session, sample_template.service, sample_template
    )
...
def test_get_all_notifications_for_job_returns_csv_format(
    ...
    assert set(notification.keys()) == set(
        [
            'created_at',
            'template_type',
            'template_name',
