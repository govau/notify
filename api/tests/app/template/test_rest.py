...
@pytest.mark.parametrize(
    ...
    [(SMS_TYPE, None), (EMAIL_TYPE, 'subject'), (LETTER_TYPE, 'subject')],
)
...
def test_must_have_a_subject_on_an_email_or_letter_template(
...
def test_should_get_a_single_template(
    notify_db, client, sample_user, service_factory, subject, content, template_type
):
...
def test_create_400_for_over_limit_content(
    client, notify_api, sample_user, sample_service, fake_uuid
):
...
def test_get_template_reply_to(
    client, sample_service, template_default, service_default
):
    auth_header = create_authorization_header()
    if service_default:
        create_letter_contact(
    ...
    template = create_template(
        service=sample_service, template_type='letter', reply_to=reply_to_id
    )
...
def test_preview_letter_template_by_id_invalid_file_type(
    sample_letter_notification, admin_request
):
...
def test_preview_letter_template_by_id_template_preview_500(
    ...
    with set_config_values(
        ...
        with requests_mock.Mocker() as request_mock:
            ...
            assert (
                'Error generating preview letter for {}'.format(
    ...
    with set_config_values(
        ...
        with requests_mock.Mocker():
            ...
            assert (
                ...
                "when calling the GetObject operation: Unauthorized".format(
                    notification.id
                )
            )
...
def test_preview_letter_template_precompiled_png_file_type(
    ...
    with set_config_values(
        ...
        with requests_mock.Mocker() as request_mock:
            ...
            mock_get_letter_pdf = mocker.patch(
                'app.template.rest.get_letter_pdf', return_value=pdf_content
            )

            mocker.patch(
                'app.template.rest.extract_page_from_pdf', return_value=pdf_content
            )
...
def test_preview_letter_template_precompiled_png_file_type_hide_notify_tag_only_on_first_page(
    ...
    with set_config_values(
        ...
        mocker.patch(
            'app.template.rest.extract_page_from_pdf', return_value=png_content
        )
        mock_get_png_preview = mocker.patch(
            'app.template.rest._get_png_preview', return_value=encoded
        )
