...
def test_should_reject_bad_phone_numbers(notify_api, sample_template, mocker):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            ...
            assert (
                'Invalid phone number: Must not contain letters or symbols'
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (6, ('Hello 6\n' 'This is an email from GOV.\u200BAU\n'), '6'),
        pytest.mark.xfail(
            (None, ('we consider None equivalent to missing personalisation'), '')
        ),
    ],
)
...
def test_should_allow_valid_sms_notification(notify_api, sample_template, mocker):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            ...
            data = {'to': '0412 345 678', 'template': str(sample_template.id)}
    ...
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            ...
            service = create_sample_service(
                notify_db, notify_db_session, limit=1, restricted=False
            )
            email_template = create_sample_email_template(
...
def test_should_block_api_call_if_over_day_limit_for_restricted_service(
    ...
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            ...
            service = create_sample_service(
                notify_db, notify_db_session, limit=1, restricted=True
            )
            email_template = create_sample_email_template(
                notify_db, notify_db_session, service=service
            )
...
def test_should_allow_api_call_if_under_day_limit_regardless_of_type(
    ...
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            ...
            service = create_sample_service(
                notify_db, notify_db_session, limit=2, restricted=restricted
            )
            email_template = create_sample_email_template(
                notify_db, notify_db_session, service=service
            )
...
def test_should_not_return_html_in_body(
    ...
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            mocker.patch('app.celery.provider_tasks.deliver_email.apply_async')
            email_template = create_sample_email_template(
                notify_db, notify_db_session, content='hello\nthere'
            )
...
def test_should_not_send_sms_if_team_api_key_and_not_a_service_user(
    notify_api, sample_template, mocker
):
    with notify_api.test_request_context(), notify_api.test_client() as client:
        mocker.patch('app.celery.provider_tasks.deliver_sms.apply_async')

        data = {'to': '07123123123', 'template': str(sample_template.id)}
...
def test_should_send_sms_to_anyone_with_test_key(
    ...
    data = {'to': '0412345678', 'template': sample_template.id}
...
@pytest.mark.parametrize('to_sms', ['+61400900000', '+61400900111', '+61400900222'])
...
def test_should_not_send_notification_to_non_whitelist_recipient_in_trial_mode(
    ...
    service = create_sample_service(
        notify_db, notify_db_session, limit=2, restricted=True
    )
    service_whitelist = create_sample_service_whitelist(
        notify_db, notify_db_session, service=service
    )
    ...
    create_sample_notification(
        notify_db, notify_db_session, template=template, service=service
    )

    data = {'to': to, 'template': str(template.id)}

    api_key = create_sample_api_key(
        notify_db, notify_db_session, service, key_type=key_type
    )
    auth_header = create_jwt_token(
        secret=api_key.secret, client_id=str(api_key.service_id)
    )
...
@pytest.mark.parametrize(
    'notification_type, to, _create_sample_template',
    [
        (SMS_TYPE, '0412 345 678', create_sample_template),
...
def test_should_send_notification_to_whitelist_recipient(
    ...
    service = create_sample_service(
        notify_db, notify_db_session, limit=2, restricted=service_restricted
    )
    apply_async = mocker.patch(
        'app.celery.provider_tasks.deliver_{}.apply_async'.format(notification_type)
    )
    ...
    if notification_type == SMS_TYPE:
        service_whitelist = create_sample_service_whitelist(
            notify_db, notify_db_session, service=service, mobile_number=to
        )
    elif notification_type == EMAIL_TYPE:
        service_whitelist = create_sample_service_whitelist(
            notify_db, notify_db_session, service=service, email_address=to
        )
    ...
    sample_key = create_sample_api_key(
        notify_db, notify_db_session, service, key_type=key_type
    )
    auth_header = create_jwt_token(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (SMS_TYPE, EMAIL_TYPE, '+61412345678'),
    ],
)
def test_should_error_if_notification_type_does_not_match_template_type(
    ...
    template = create_sample_template(
        notify_db, notify_db_session, template_type=template_type
    )
    data = {'to': to, 'template': template.id}
    auth_header = create_authorization_header(service_id=template.service_id)
    response = client.post(
...
def test_create_template_raises_invalid_request_when_content_too_large(
    notify_db, notify_db_session, template_type, should_error
):
...
@pytest.mark.parametrize(
    ...
    [("sms", "0412 345 678"), ("email", "sample@email.com")],
)
...
def test_should_allow_store_original_number_on_sms_notification(
    ...
    data = {'to': '+(61) 412 345 678', 'template': str(sample_template.id)}
    ...
    assert '+(61) 412 345 678' == notifications[0].to
...
def test_should_not_allow_international_number_on_sms_notification(
    ...
    assert (
        error_json['message']['to'][0] == 'Cannot send to international mobile numbers'
    )
...
def test_should_allow_international_number_on_sms_notification(
    ...
    service = sample_service(
        notify_db, notify_db_session, permissions=[INTERNATIONAL_SMS_TYPE, SMS_TYPE]
    )
...
def test_should_not_allow_notification_if_service_permission_not_set(
    ...
    mocked = mocker.patch(
        'app.celery.provider_tasks.deliver_{}.apply_async'.format(
            template_without_permission.template_type
        )
    )
    ...
    auth_header = create_authorization_header(
        service_id=template_without_permission.service_id
    )
...
@pytest.mark.parametrize(
    "notification_type, recipient", [("sms", '0412 345 678'), ("email", "test@gov.au")]
)
def test_post_notification_should_set_reply_to_text(
    client, notify_db, notify_db_session, mocker, notification_type, recipient
):
    mocker.patch(
        'app.celery.provider_tasks.deliver_{}.apply_async'.format(notification_type)
    )
    ...
    if notification_type == EMAIL_TYPE:
        ...
        create_reply_to_email(
            service=service, email_address=expected_reply_to, is_default=True
        )
