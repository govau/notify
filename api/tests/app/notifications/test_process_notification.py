...
@pytest.mark.parametrize("to_address, notification_type, expected", [
    ...
    ("0400900000", "sms", True),
    ("0400900111", "sms", True),
    ("simulate-delivered@notify.digital.gov.au", "email", True),
    ("simulate-delivered-2@notify.digital.gov.au", "email", True),
    ("simulate-delivered-3@notify.digital.gov.au", "email", True),
...
def test_persist_scheduled_notification(sample_notification):
    ...
    assert scheduled_notification[0].scheduled_for == datetime.datetime(2017, 5, 12, 4, 15)
