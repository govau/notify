...
def test_service_can_send_to_recipient_passes_for_whitelisted_recipient_passes(notify_db, notify_db_session,
    ...
    sample_service_whitelist(notify_db, notify_db_session, mobile_number='0423332413')
    assert service_can_send_to_recipient('0423332413',
                                         'team',
                                         sample_service) is None
...
@pytest.mark.parametrize('recipient', ['0413332413', 'some_other_email@test.com'])
@pytest.mark.parametrize('key_type, error_message',
                         [('team', 'Can’t send to this recipient using a team-only API key'),
                          ('normal',
                           "Can’t send to this recipient when service is in trial mode – see https://notify.gov.au/features/using-notify")])  # noqa
...
def test_service_can_send_to_recipient_fails_when_mobile_number_is_not_on_team(notify_db, notify_db_session):
    ...
    with pytest.raises(BadRequestError) as e:
        service_can_send_to_recipient("0458964221",
                                      'team',
                                      live_service)
    assert e.value.status_code == 400
