...
def test_check_service_message_limit_in_cache_over_message_limit_fails(
    notify_db, notify_db_session, key_type, mocker
):
    with freeze_time("2016-01-01 12:00:00.000000"):
...
def test_service_can_send_to_recipient_passes(key_type, notify_db, notify_db_session):
    trial_mode_service = create_service(
        notify_db, notify_db_session, service_name='trial mode', restricted=True
    )
    assert (
        service_can_send_to_recipient(
            trial_mode_service.users[0].email_address, key_type, trial_mode_service
        )
        is None
    )
    assert (
        service_can_send_to_recipient(
            trial_mode_service.users[0].mobile_number, key_type, trial_mode_service
        )
        is None
    )
...
def test_service_can_send_to_recipient_passes_for_live_service_non_team_member(
    ...
    live_service = create_service(
        notify_db, notify_db_session, service_name='live', restricted=False
    )
    assert (
        service_can_send_to_recipient(
            "some_other_email@test.com", key_type, live_service
        )
        is None
    )
...
def test_service_can_send_to_recipient_passes_for_whitelisted_recipient_passes(
    ...
    sample_service_whitelist(
        notify_db, notify_db_session, email_address="some_other_email@test.com"
    )
    assert (
        service_can_send_to_recipient(
...
@pytest.mark.parametrize(
    'recipient',
    [{"email_address": "some_other_email@test.com"}, {"mobile_number": "07513332413"}],
)
def test_service_can_send_to_recipient_fails_when_ignoring_whitelist(
    notify_db, notify_db_session, sample_service, recipient
):
...
def test_service_can_send_to_recipient_fails_when_recipient_is_not_on_team(
    recipient, key_type, error_message, notify_db, notify_db_session
):
    trial_mode_service = create_service(
        notify_db, notify_db_session, service_name='trial mode', restricted=True
    )
...
def test_service_can_send_to_recipient_fails_when_mobile_number_is_not_on_team(
    ...
    live_service = create_service(
        notify_db, notify_db_session, service_name='live mode', restricted=False
    )
    with pytest.raises(BadRequestError) as e:
...
def test_check_sms_content_char_count_fails(char_count, notify_api):
    ...
    assert (
        e.value.message
        == 'Content for template has a character count greater than the limit of {}'.format(
...
def test_that_when_exceed_rate_limit_request_fails(
    ...
    with freeze_time("2016-01-01 12:00:00.000000"):
        ...
        api_key = sample_api_key(
            notify_db, notify_db_session, service=service, key_type=api_key_type
        )
...
def test_that_when_not_exceeded_rate_limit_request_succeeds(
    ...
    with freeze_time("2016-01-01 12:00:00.000000"):
        ...
        service = create_service(notify_db, notify_db_session, restricted=True)
        api_key = sample_api_key(
            notify_db, notify_db_session, service=service, key_type='normal'
        )
...
def test_allows_api_calls_with_international_numbers_if_service_does_allow_int_sms(
    ...
    service = create_service(
        notify_db, notify_db_session, permissions=[SMS_TYPE, INTERNATIONAL_SMS_TYPE]
    )
    result = validate_and_format_recipient(
        '20-12-1234-1234', key_type, service, SMS_TYPE
    )
...
def test_check_service_letter_contact_id_where_letter_contact_id_is_found(
    ...
    letter_contact = create_letter_contact(
        service=sample_service, contact_block='123456'
    )
    assert (
        check_service_letter_contact_id(
            sample_service.id, letter_contact.id, LETTER_TYPE
        )
        == '123456'
    )
...
def test_check_service_letter_contact_id_where_service_id_is_not_found(
    ...
    assert (
        e.value.message
        == 'letter_contact_id {} does not exist in database for service id {}'.format(
