...
def firetext_post(client, data, auth=True, password='testkey'):
    ...
    if auth:
        auth_value = base64.b64encode(
            "notify:{}".format(password).encode('utf-8')
        ).decode('utf-8')
        headers.append(('Authorization', 'Basic ' + auth_value))

    return client.post(
        path='/notifications/sms/receive/firetext', data=data, headers=headers
    )
...
def mmg_post(client, data, auth=True, password='testkey'):
    ...
    if auth:
        auth_value = base64.b64encode(
            "username:{}".format(password).encode('utf-8')
        ).decode('utf-8')
        headers.append(('Authorization', 'Basic ' + auth_value))

    return client.post(
        path='/notifications/sms/receive/mmg', data=json.dumps(data), headers=headers
    )
...
@pytest.mark.parametrize('permissions', [[SMS_TYPE], [INBOUND_SMS_TYPE]])
def test_receive_notification_from_mmg_without_permissions_does_not_persist(
    ...
    create_service_with_inbound_number(
        inbound_number='07111111111', service_permissions=permissions
    )
    data = {
        "ID": "1234",
        "MSISDN": "07111111111",
...
def test_check_permissions_for_inbound_sms(
    notify_db, notify_db_session, permissions, expected_response
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('\x79 \\x79 \\\\x79', 'y y \\x79'),  # we should never see the middle one
    ],
)
...
def test_receive_notification_from_firetext_persists_message_with_normalized_phone(
    notify_db_session, client, mocker
):
    mocker.patch(
        "app.notifications.receive_notifications.tasks.send_inbound_sms_to_service.apply_async"
