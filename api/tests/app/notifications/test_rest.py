...
def test_get_all_notifications(client, sample_notification):
    ...
    assert notifications['notifications'][0]['to'] == '+61412345678'
...
def test_normal_api_key_returns_notifications_created_from_jobs_and_from_api(
    ...
    api_notification = create_sample_notification(
        notify_db, notify_db_session, api_key=sample_api_key
    )
...
def test_get_all_notifications_only_returns_notifications_of_matching_type(
    ...
    team_notification = create_sample_notification(
        notify_db, notify_db_session, api_key=team_api_key, key_type=KEY_TYPE_TEAM
    )
    test_notification = create_sample_notification(
        notify_db, notify_db_session, api_key=test_api_key, key_type=KEY_TYPE_TEST
    )
...
def test_get_all_notifications_newest_first(
    ...
    notification_1 = create_sample_notification(
        notify_db, notify_db_session, sample_email_template.service
    )
...
def test_valid_page_size_param(
    notify_api, notify_db, notify_db_session, sample_email_template
):
