...
def test_get_s3_bucket_objects_builds_objects_list_from_paginator(notify_api, mocker):
    ...
    multiple_pages_s3_object = [
        {"Contents": [single_s3_object_stub('bar/foo.txt', AFTER_SEVEN_DAYS)]},
        {"Contents": [single_s3_object_stub('bar/foo1.txt', AFTER_SEVEN_DAYS)]},
    ]
    paginator_mock.return_value.get_paginator.return_value.paginate.return_value = (
        multiple_pages_s3_object
    )
...
def test_get_list_of_files_by_suffix(
    notify_api, mocker, suffix_str, days_before, returned_no
):
    paginator_mock = mocker.patch('app.aws.s3.client')
    multiple_pages_s3_object = [
        {
    ...
    if days_before:
        ...
        key = get_list_of_files_by_suffix(
            'foo-bucket', subfolder='bar', suffix=suffix_str
        )
...
def test_get_list_of_files_by_suffix_empty_contents_return_with_no_error(
    notify_api, mocker
):
    paginator_mock = mocker.patch('app.aws.s3.client')
    multiple_pages_s3_object = [{"other_content": ['some_values']}]
    paginator_mock.return_value.get_paginator.return_value.paginate.return_value = (
        multiple_pages_s3_object
    )
