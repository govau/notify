...
def test_get_platform_stats_uses_todays_date_if_no_start_or_end_date_is_provided(admin_request, mocker):
    ...
    dao_mock.assert_called_once_with(start_date=today, end_date=today, include_from_test_key=True)
...
def test_get_platform_stats_can_filter_by_date(admin_request, mocker):
    ...
    dao_mock.assert_called_once_with(start_date=start_date, end_date=end_date, include_from_test_key=True)
